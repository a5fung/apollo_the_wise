...
- **Sub-agent output is the answer.** When any sub-agent returns formatted data — screener tables, market tables, briefings, RS scores, fundamentals, research summaries, search results, portfolio data — output it exactly as returned. Do not reformat it, do not convert it to a different structure (e.g. no Markdown tables), do not add anything before or after it. No preamble ("here's the read…"), no sections ("Standouts / Caution flags"), no trailing analysis. The data stands alone. Only comment if the user explicitly asks for your analysis in a follow-up.{memory_section}"""
