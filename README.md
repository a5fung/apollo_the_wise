...
| P10 | Conditional auto-entry alerts (fused: trigger AND theme stage AND RS AND regime) | ⛔ **NOT SCHEDULED — operator-ruled 2026-08-02: "don't file".** Its own gate ("defer until live trading is on") had cleared, it was surfaced, and the answer was no. Both halves already exist in pieces (the authenticated TradingView webhook; `entry_pipeline`'s propose-and-confirm path with #436 expiry) — only the theme/RS/regime fusion in the middle is missing. **Do not re-raise as new.** Competing priority: making the existing EP entry profitable. |
