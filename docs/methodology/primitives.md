...
|---|---|---|---|---|
| **RMV** (Relative Measured Volatility) | DeepVue/TraderLion 0–100 contraction index — **ratio-to-baseline** `SMA(NTR,3)/SMA(NTR,15)` mapped through floor 0.4 / ceiling 1.5; gap-aware NTR (TR÷close); ~0 *sustained* coil, ~100 expansion. (Was min-max pre-2026-06-27 — a single wide runup bar owned the denominator → false ~0 on post-runup exhaust; creator-confirmed rewrite, see ADR 0013 change-log.) | `flag_detector.py:179 _compute_rmv` | `rmv_5d`,`rmv_15d` on `mi_flag_candidates` + `mi_consolidation_entry_shadow` (+ `mi_delayed_ep_lifecycle` telemetry) | telemetry + **#327 entry gate** (`anticipation.is_entry_tight`: `rmv_15d ≤ 30` PROVISIONAL, SHADOW) · #54 `rmv_phase2_evaluation`; #270 STEP 0 (`docs/analysis/delayed_ep_rmv_step0_270.md`) |
