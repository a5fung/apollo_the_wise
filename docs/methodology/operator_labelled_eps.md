...
1. Add it to `shared/operator_labelled_eps.py::OPERATOR_LABELLED_EPS` — ticker + alert_date only,
   OPERATOR-AUTHORED, never inferred. This is the identity `agents/` code checks against.
2. Add the corresponding member to `tests/fixtures/must_not_miss_eps.py`
   (`label_source="operator"`, his words) — it sources its `ticker`/`alert_date` FROM the module
   above, not a second hand-typed copy.
3. Add a row here. `tests/test_operator_labelled_ep_list.py` fails the build if any of the three
   (this doc, the shared module, the fixture) disagree.
4. Record what our system actually did with it — that is the whole point of the list.

Never add a name because it worked. Never remove one because it didn't.

**2026-09-07**: `agents/market_intelligence/audit_invariants.py::check_operator_ep_downgraded`
watches every name above for a catalyst-rubric downgrade automatically, on every scheduled audit
sweep — the operator no longer has to notice it himself. A downgrade gets silenced ONLY via
`shared/operator_labelled_eps.py::ACKNOWLEDGED_DOWNGRADES`, keyed by (ticker, alert_date, the
specific downgrade event type) and requiring a named root cause plus the task that owns it — never
"we looked and it's fine". Two entries today: BFLY/#210 (below) and TEAM/#448 (the
2026-08-06/08-07 extraction-outage bug, already operator-ruled and already fixed — see that
module's comment for the full trace).
...
- `shared/operator_labelled_eps.py` — the importable identity list (ticker + alert_date only) plus
  the acknowledged-downgrade set. `agents/` code imports THIS, never the fixture below.
- `tests/fixtures/must_not_miss_eps.py` — the machine-readable fixture (33 members; these 7 plus 26
