...
- **APPS added to the (a2) recovery cohort (operator 2026-05-31):** Digital Turbine — beaten-down mobile-software/adtech re-rating off a low base, the same *recovering*-software shape as ESTC/TEAM/MDB (NOT a same-day co-gap/drone case → it tests `get_rs_recovery_slope`, not the co-gap path). Two uses Monday: (i) **selector verification (#160)** — confirm APPS's `rs_1m`/`rs_6m` from the prod read and assert it ENTERS the recovery-slope set; (ii) **threshold-tuning data point (#162)** — if its `rs_6m` lands in 30–40 it joins NOW(33)/MDB(20) as a vote to loosen the seeded `rs_1m≥90 ∧ rs_6m≤30` cut toward `rs_1m≥85 ∧ rs_6m≤40`. Also feed APPS into the §4-step-2 grouping leg with the software cohort. (RS numbers deliberately unfilled — to be read from prod, not asserted.)
