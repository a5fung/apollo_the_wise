...
| **GATE 3 — paper R expectancy** | ✅ GO-SUPPORTIVE (reduced-size) | **Primary output `sip_replay_r_cohort.py`, re-run 6/19 (read-only, no LLM):** three cohort slices, NOT one N — (a) **raw paper-IEX filled** N=21 = −5.64R/33% (≈ −$9,475 $-baseline) = the trades IEX *did* fill, the **selection artifact**; (b) **SIP REAL-only** N=23, E[R] **−0.45** = the same real trades repriced on SIP/NBBO; (c) **synth-CANCELLED** N=15, E[R] **+1.183**, totR +17.74, win 6/15 = the breakouts IEX *cancelled* but SIP would have filled. Selection delta = synth-CANCELLED +1.18 vs synth-FILLED −1.00 = **+2.18R** (IEX fills losers, drops winners). Reachability: gap_through=0 (100% reachable), 6/6 winners convincing fills. **Concentration:** drop-top-1 +0.748, drop-top-2 +0.417, drop-top-3 **+0.087**, drop-top-4 **−0.291 (crosses 0)** → edge survives drop-3 but barely → **START-SMALL**. (memory `project_gate3_sip_selection_finding`, `paper_iex_vs_live_sip_gate_adjustment`.) |
