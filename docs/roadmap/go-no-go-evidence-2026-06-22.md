...
corpus-completeness fix shipping as a **premarket shadow now + the live flip Mon/Tue (#347)**
SATISFIES the gate — the live flip need not have landed before GO. This remains **conditional
on Monday's shadow verify (#346) being clean** (a bad shadow read is a NO-GO trigger); it is
NOT a waiver of the #347 CHANGE_PROCESS + sign-off on the flip itself.
