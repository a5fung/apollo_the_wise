...
1. **#65 — 9M Day-2: add the revenue-stage gate now.** ~~Mirror MAGNA53's `is_revenue_stage` on `9m_day2` → kills the clinical-biotech class (ROIV/PURR).~~ **⚠️ REFUTED 6/2 by CHANGE_PROCESS evidence (read-only):** `is_revenue_stage` at the $0.01 pin **keeps** ROIV (rev $2.37M) / PURR ($3.57M) / all 25 sampled candidates → **no-op, kills nothing.** The premise is false — ROIV/PURR aren't pre-revenue (they have $2–3.6M revenue); revenue-stage is the wrong filter for binary-catalyst risk; and the threshold is a single env var **shared with MAGNA53** + the contested #55 pin (6/20), so it can't be raised for 9m_day2 alone. → **DECIDED 6/2 (operator): LEAVE 9m_day2 UNCHANGED.** No filter change (option b was a no-op); 9m_day2 keeps paper-trading the Day-2 ORB as-is, accruing to the 7/15 #65 review. A biotech-sector filter or the #55 threshold remain *future* options, not now. See task #65.
