...
| **`docs/analysis/562_delayed_entry_trigger_2026-09-01.md`** | **1,269 campaigns / 1,017 walked — the LIVE lane's first night, non-outcome-conditioned** | 🔑 **IT INVERTS THE PREMISE THIS TASK WAS BUILT ON.** *"1 of 104 ever reached TRIGGERED"* described the FLAG stage machine the operator ruled out — the actual delayed-entry rungs fire on **91% of watched names (925 of 1,017) within ≤5 sessions**, 59% on two or more. Nothing is failing to fire. **Names die AT THE STOP instead: 1,292 of 1,293 settled first-fires are stop-outs, the median pullback-rung stop-out landing the SAME DAY it fired, at stops 2.3–2.9% of entry.** Same mechanism the 08-22 study found on 44 stopped-out episodes, now on a 1,018-campaign population that was never selected on outcome. ▶ **Binding constraint = a MISSING SELECTION RULE, not a threshold and not a data gap** (enrollment recall 100%, bar gaps 0.7%): nothing decides which of ~350 fires/day to take. ⚠ **And the lane's own selection instrument is DARK — `screen_member` is stamped on 2 of 1,269 campaigns.** ⚠ **Counts re-run independently against prod before this row was written**, not taken on the card's word. **No p, no ranking, no expectancy from one night — the honest first outcome read is ~2026-09-23**, NOT when the 30-settled gate trips (settled-only rows today are all losers by construction, because the open rows are the candidate winners). |
