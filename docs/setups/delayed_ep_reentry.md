...
🔒 **Mechanically held** — by `tests/test_ssot_router_complete.py::test_declared_topics_carry_their_findings`, which FAILS the build when a delayed-entry analysis or design document exists that this ledger does not reference. A finding cannot become orphaned by inertia, which is how this context was lost the first time.
⚠ **This line used to name `tests/test_delayed_entry_ledger_complete.py`, which HAS NEVER EXISTED IN THIS REPO** (found 2026-09-01). The gate itself is real and did catch the #562 finding above — only the pointer was fiction. A claimed gate nobody can find is the same class of rot this ledger exists to prevent, so the name is now the real test's.
...
| **`docs/analysis/delayed_entry_stop_grid_2026-09-01.md`** | **all 4 signals × 13 stop bases on the same 267 campaigns; 602/602 fires reproduced, incumbent settlements 0-drift** | 🔑 **ONE SHAPE, AND IT IS THE ANSWER: THE WORKING STOP IS VOLATILITY-PROPORTIONAL, NOT STRUCTURAL.** Every incumbent sits OUTSIDE the 0.75–1.25×ADR band — the reclaims and 620 at 0.34–0.50×ADR (noise; median fire is a full stop), high-break at 1.91×ADR (winners barely pay) — and moving into the band improves every rung *from either direction*, peaking at 0.75–1.0×ADR and decaying beyond. ✅ **THE ONE POSITIVE CELL FAMILY: `ep_low_reclaim` × entry−0.75..1.0×ADR × trailing exit.** Verified independently: n=130, **+0.17 to +0.25R per fire pooled, +0.16 to +0.19R EX-MAY (n=70)** — the only family positive in May AND June AND July — and it **kills ZERO fires**, so recall is untouched. ⚠ **AND IT KILLS MY OWN EARLIER HIGHLIGHT: `ep_high_break` × adr_025 reads +1.18R pooled but +0.02R ex-May on n=17** — the +53R was VPG and ARM, two May gap-overs. **Do not build on it.** ❌ Dead: `ep_close_reclaim` everywhere (best cell is one-fire-carried); every bar-anchored stop (dominated on all four rungs, 119/119 full stops on 620, kills VPG/ARM); EP-close as a stop for pullback rungs (kills 176 mature fires including EVERY big reclaim winner — STUB, TE, ABVX, GO, BHVN, NAVN); EP-low under low_reclaim (0.06×ADR, −0.89R, worst on the board). 📉 **FAMILY-WIDE the lever is real but modest: −0.18 → +0.05R per fire, holding ex-May, ex-May ≥4R tail 6 → 15. It turns "clearly losing" into "about break-even" — not a business.** Selection and harvest still decide profit (25 fires TOUCHED ≥4R; only 13 kept it on hold-only, 6 on the trail). ⚠ ~104 cells scanned per arm — **the evidence is the monotone band shape across four rungs and three months, not any single cell**; knife-edge is real (ABVX +14.1R vs −1.0R across 20 cents of stop). **A candidate for a forward out-of-sample read, NOT a proven edge.** ⚖ Nothing flipped. |
| **`docs/analysis/545_selection_test_2026-09-01.md`** | **569 settled fires, 18 tails, 10 of 11 PRE-REGISTERED features, 49 cuts** | 🛑 **THE NULL, AND IT WAS WRITTEN DOWN BEFORE THE TEST RAN. Nothing knowable at fire time separates the tail.** Base tail rate 3.2%; the bar was ≥8% at n≥30 holding ex-May and on both exit arms. **Zero of 49 cuts passed.** The only cut over 8% with n≥30 is *only one signal fired* (13.3%) and **May alone carries it — 4.5% once May is removed**, the same collapse every other delayed-entry result showed. Mean R is negative in every cut but an n=8 curio. RS, gap size, dollar volume, catalyst grade, theme membership, which signal, when it fired — all land 3-6% against a 3.2% base. ⚠ **49 cuts on 18 positives means a SINGLE passing cut would have been noise; zero passing is the stronger statement.** ⚠ Extension at the EP was untestable (0 of 569 fires carry it — the same darkness that killed `screen_member`). ▶ **RECOMMENDATION — CORRECTED the same day: stop hunting for a FILTER, not for the tactic.** My first version said kill delayed entry; the operator reframed it and he was right — *"we are not predicting anything… we find entries and exits where we can manage risk properly and where we have positive expected returns."* This study tested PREDICTION and the answer is no. It did not test profitability, and the evidence already in hand says the STOP moves it far more than any filter: the same 32 `ep_high_break` fires go **−0.41R → +1.66R** purely by moving the stop from the prior session low (10.8% of entry) to entry − 0.25×ADR (1.4%), and `ep_close_reclaim` goes −0.44R → −0.06R on the trailing arm with ≥4R rising 2 → 8. **The stop sweep has been run for ONE of the four signals. Do that for all four before any verdict on the tactic.** ⚖ Nothing flipped. |
| **`docs/analysis/delayed_entry_theoretical_day1_2026-09-01.md`** | **157 of 267 caught EPs that MET OUR ENTRY CRITERIA, with the day-1 ORB trade SIMULATED for every one** | ⚖ **OPERATOR RULING that forced it:** *"the names we didn't enter can be for reasons beyond the stock itself, like we hit our cap, or the window beyond 9:45… we should just look at all EPs that meet our criteria"* — "did we enter" mixed what the STOCK did with whether WE had capacity and timing, and **111 of the 157 (71%) had no fill for OUR reasons** (window-late 38 · order unfilled 34 · no attempt 9 · key outage 8 · breaker 7 · cap 5 · size 5). 🔴 **IT ALSO CAUGHT AN INSTRUMENT ERROR IN THE STUDY ABOVE: our live ORB is the FIRST 1-MINUTE BAR** (`entry_pipeline.py:99`, `alpaca_client.get_first_bar`), **not the 9:30-9:45 range** that study measured against. Validated 13/13 against real fills. Under the correct level only **12 of 157 never trigger** — the earlier "94 never broke the ORB high" was mostly the wrong level. **Simulator agreement with our real day-1 trades: 33 of 34 (97%)**, sole miss MANE on a pessimistic intra-bar convention; 25 abstains counted, ALAB among them (±34R, stated not guessed). 🔑 **AND THE RANKING INVERTS — the 09-01 recommendation built on the old split is WITHDRAWN.** Simulated stopped-out-on-day-1 (68 campaigns) is −0.45R/fire and holds the biggest in-population tail (ANF +7.3, FET +7.1, VPG +6.2, NAVN +4.9; the trail arm adds BLZE, FTNT, KLAR), with **4 of 7 tail fires OUTSIDE May**; never-triggered is −0.79R with ZERO tail (n=12, thin). Most of the backfill's recorded tail (TE, GO, BHVN, MMYT, QBTS) sits on names that never met the day-1 criteria at all. ▶ **Still no positive group — the median fire is a full stop everywhere, so SELECTION (P13) remains the binding constraint and day-1 group membership is at most a ranking signal.** ⚖ Nothing flipped. |
| **`docs/analysis/545_retry_test_2026-09-02.md`** | **#545 Phase 1, THE RETRY TEST — pre-registered grid (stop 0.25/0.50/0.75/1.00×ADR × attempts 1/2/3/unlimited × both exit arms) chained through the lane's own two re-entry shapes on the same 267 campaigns; attempts=1 reproduces the 09-01 stop grid on all 4,816 rows, 0 drift; per-NAME accounting at equal dollars per attempt** | 🔑 **THE OPERATOR'S TIGHT-STOP-MORE-TRIES IDEA MEETS THE PRE-REGISTERED BAR ON ITS LETTER — on ONE rung, in ONE cell family, carried by TWO names.** `ep_low_reclaim` × entry−0.25×ADR × up to 3 tries × trailing exit: **+54.4R across 46 ex-May names vs +5.3R for one try at 0.75×ADR on the same 46**; worst name −3.0R vs −1.0R (exactly the two extra tries' nominal risk); survives drop-best (+18.2R); positive in June AND July. **The kill condition did not fire.** ⚠ **What it rests on:** NRIX (+36.3R after two −1R cuts, third try 06-11 hand-walked in raw bars) and EFOR (+33.2R after one) are +69.5R of it; 34 of 46 names lose; 11 of 46 spend all three stops and never position; the −3R worst is the −1.0R convention's number — **charging the 8 gap-through stops at the open it is +43.7R with the worst name −5.8R, which FAILS the bar's drawdown clause**; at live sizing the 20% notional cap makes a 1.5% stop worth a third of a dollar-R (+14.9 vs +7.4 R-eq). ❌ Retries SUBTRACT on `ep_close_620_prox` (every retry column worse than one try at 0.75×ADR, +11.4R), `ep_close_reclaim`'s one good-looking cell is FET alone, `ep_high_break` has 9 readable ex-May names. ⚠ Limitation the pre-registration did not foresee: chained windows run past the lane's 20-session minute capture → 783 blind (ticker, day) pairs, 4 of the 46 headline names touched (`_545rt_blind_capture.sql` pulls the 162 prod holds). ▶ **Status: CANDIDATE for the forward out-of-sample read (record the 0.25×ADR variant beside #616's two), not an edge.** ⚖ Nothing flipped. |
| **`docs/analysis/delayed_entry_stop_and_population_2026-09-01.md`** | **the same 267 caught EPs, split by what happened on day 1; plus 48 high-break fires under 8 stop bases** | 🔑 **THE NAMES THAT KNOCKED US OUT ARE THE WORST PLACE TO GO BACK IN.** Split (my own count re-run from the classification file): **entered day 1 and stopped out 41 · never broke the ORB high 94 · entered and still running 4 · BROKE THE ORB HIGH BUT WE NEVER ENTERED 124 · unclassifiable 4.** Delayed entry on the stopped-out group is **−0.71R per fire (n=60), 1 fire in 60 reaching 4R — worst of every group, in every month, under both exits.** The never-entered group is the least bad (−0.29, trail +0.01) and holds half the tail. **Being stopped out on day 1 is itself evidence the name has no tail** — the 08-29 stopped-cohort finding, now on 3× the n. ▶ **STOP WIDTH ON THE HIGH BREAK (operator: *"stop for high break needs to be tighter"*): tighter is right, but NOT anchored on the bar's own low.** Breakout-bar-low and low-of-day kill **21 of 48 fires at birth — including BOTH winners (ARM +4.45R, VPG +6.17R), because winners GAP OVER the EP high**, so their fire bar's low is already far beneath. Anchoring on ADR instead (entry − 0.25×ADR) or the EP-day close kills nothing, cuts the stop 10.8% → 1.4-2.2%, and turns −13R into **+53R**. ▶ **He also asked about "the entry day low" — BOTH readings are now measured and neither helps.** The low of the session the entry fires is the TEAM basis above (kills 21 of 48). The EP DAY's own low is **not tight at all: 11.1% median stop, mean −0.39R — statistically the incumbent** (10.8%, −0.41R), because on a big gap day the gap-day low sits about as far under the gap-day high as the prior session's low does. **Only ADR-anchored or EP-close bases tighten without killing fires.** ⚠ **AND THAT +53R IS TWO MAY NAMES.** June −8R and July −9R under the same tight stop; every month but May is a total loss, and May is the era he ruled stale. **n(winners)=2 — this RANKS the basis families and cannot pick a value.** ⚠ 105 of the 124 never-entered names had NO ORDER AT ALL, and **every one was detected outside the 9:31-9:45 submission window** (59 of them HIGH tier) — that is the ORB-window question **#587 already owns**, not a delayed-entry one, and a late detection is not the same as a missed entry. ⚖ Nothing flipped. |
| **`docs/analysis/delayed_entry_backfill_2026-09-01.md`** | **267 caught EPs replayed from stored bars, May-Aug (74·53·41·99) — the RIGHT population, ~570 settled fires** | 🔑 **RECALL IS NOT THE PROBLEM AND NEVER WAS. The family fires on 257 of 267 caught EPs (96%) and on 13 of the 13 that ran ≥8×ADR — it finds the turns.** ▶ **AND IT LOSES MONEY AS DEFINED: every rung is mean-negative, median −1.00R — i.e. THE TYPICAL FIRE IS A FULL STOP-OUT.** Mature fires: low-reclaim −0.39 (n=130) · close-reclaim −0.44 (n=86) · high-break −0.41 (n=32) · 620-prox −0.35 (n=126); win rate 6-12%; **18 of ~570 fires ever harvested ≥4R.** Pooling the immature August rows (biased negative by construction) it is −0.60/fire, −342R total — I recomputed that from the raw fire rows rather than take the card's word. ⚠ **THE WHOLE TAIL IS MAY, THE ERA HE RULED STALE:** the six biggest winners are ALAB +21.6/+12.3, STUB +11.4/+8.9, TE +10.7/+8.3 — every one a May campaign; drop the top three and the sum falls from −342R to −388R. **August is unreadable on purpose** — its settled fires are stops by construction because the winners are still open; reads ~late Sept. ▶ **SO THE MISSING PIECE IS SELECTION AND RE-ENTRY, NOT RUNG DEFINITIONS** — at a 10% win rate the average winner must clear 8.5R and clears 4.74R; no tier or grade cut closes it. **5 of the 13 monsters (NRIX, ALOY, EFOR, HLIT, AKTS) had every first fire stop out BEFORE the run** — the re-entry lens, not replayed here. ⚖ Nothing flipped; any rung change is CHANGE_PROCESS + sign-off. |
| **`docs/analysis/562_delayed_entry_trigger_2026-09-01.md`** | 🛑 **NUMBERS VOID — WRONG POPULATION** (1,269 campaigns, only SIX of them EP alerts; 816 gapped under our own 9% floor). Mechanism survives, percentages do not | 🔑 **IT INVERTS THE PREMISE THIS TASK WAS BUILT ON.** *"1 of 104 ever reached TRIGGERED"* described the FLAG stage machine the operator ruled out — the rungs fired constantly and died at the stop almost immediately — but on GAPPERS, not caught EPs, so **no rate from this study may be quoted.** ⚠ Its one durable contribution is the DESCRIPTION of the four rungs (buy point, stop, gates, abstain rule), read from the code. ▶ The same rungs are replayed over the 267 EPs our system actually caught (May-Aug) in `docs/analysis/delayed_entry_backfill_2026-09-01.md` — read that for any number. Same mechanism the 08-22 study found on 44 stopped-out episodes, now on a 1,018-campaign population that was never selected on outcome. ▶ **Binding constraint = a MISSING SELECTION RULE, not a threshold and not a data gap** (enrollment recall 100%, bar gaps 0.7%): nothing decides which of ~350 fires/day to take. ⚠ **And the lane's own selection instrument is DARK — `screen_member` is stamped on 2 of 1,269 campaigns.** ⚠ **Counts re-run independently against prod before this row was written**, not taken on the card's word. **No p, no ranking, no expectancy from one night — the honest first outcome read is ~2026-09-23**, NOT when the 30-settled gate trips (settled-only rows today are all losers by construction, because the open rows are the candidate winners). |
| **`docs/analysis/missed_ep_population_327_2026-08-29.md`** | **55 real EPs that got past us on day 1** | **The population the ladder was never measured on**, built because #562 only ever ran on names we ENTERED and were stopped out of. ⚠ **CORRECTED same day — the 22 attributed to the top-20-by-gap cap is a rule that NO LONGER EXISTS** (replaced 2026-08-22 by the liquidity-led pre-score; toggle confirmed on in prod). Historically true, operationally useless — re-measure before citing any per-cause count. Causes: **top-20-by-gap cap 22** · old grading/score 7 · RVOL 3 · entry-blocked 3 · silent floors 3 · infra 3 · extension 3 · mcap 2 · M&A 1 · unknowable 4 · misc 4 · provisional 6. 🔑 **THE COHORTS DIFFER MATERIALLY, so #562's ranking does NOT transfer:** the ≥10R missed EPs gapped **8.7–11.1%** against the stopped cohort's **12.1–20.9%**, and the stopped 44 were **~93% tail-free**. We were stopped out of names that had no tail; we MISSED the ones that did. Also settles half the 620@EPC band: **SMCI closed +4.19R** (verified against the study's +5.45 mark), giving **+0.45R over n=30 settled — about zero per trade, median −1.00R** — through +8.18R with TEAM's open mark. **TEAM settles 09-08; no conclusion from the open mark.** ⚠ Its own stated weakness: the label is outcome-conditioned, so real EPs that FAILED are invisible to it. |
| **`docs/analysis/delayed_entry_stage2_327_2026-08-30.md`** | **the missed real EPs, 43 of 55 evaluable, minute bars** | **#562's ranking does NOT transfer — it inverts.** Same instrument both cohorts (9 anchor trades + 3 full arms reproduce #562 exactly; 620-arm tolerance ±0.4R stated): on missed real EPs every arm is mean-positive; the stopped cohort's worst arm (EPL-UR, the MNTS undercut blueprint) is top by mean (+5.79R, ALMU-carried; +2.00 ex-top-1), **EPC-REC is the robust one** (median +1.02, 57% win, +1.76 ex-top-1) and EPH-BRK fires on 95% (median +0.26, best-in-class ADR units with EPC-REC). **Fire-rate inversion:** pullback arms fire 14–53% on real EPs vs 55–95% on stopped names — the strong names don't come back (the pivot-proximity tension, now measured on the miss side). Union ceiling: 12/43 events bank ≥4R. 72 of 80 pullback-arm full stops sat in front of a ≥4R run — stop geometry, not location, still binds. ⚠ Label is outcome-conditioned (all numbers inflated by construction; the stopped table is the bleed proxy) and 12 events are DARK (no bars — the April cap-kill class; coverage 64% and correlated). |
| **`docs/analysis/delayed_entry_stage2_327_2026-08-30.md`** | **the 55 MISSED EPs (Stage 2)** | 🔑 **#562's RANKING DOES NOT TRANSFER — IT INVERTS.** Same eight triggers, same definitions, same harness; only the population changed. On the missed cohort: EPL-UR **+5.79R mean / −1.00 median (n=13)** — #562's WORST arm is now top by mean — 620@EPL +2.57 · **EPC-REC +2.26 mean and +1.02 MEDIAN (n=23) — the only arm with a positive median anywhere in either study, and the robust one** · 620@PDH +1.83 · 620@EPC +1.40 (was the sole positive at +0.21) · 620-ANY +1.18 · EPH-BRK +0.72 (n=41) · 620@MA10 +0.38. **12 of 43 events bank ≥4R via the best arm.** EPL-UR's mean is ALMU-carried (+2.00 ex-top-1). Instrument validated: reproduces 9 of #562's anchors exactly. ⚠ **THE CAVEAT IS STRUCTURAL, not a footnote: the "real EP" label is OUTCOME-CONDITIONED, so every number here is inflated by construction — real EPs that FAILED are invisible to the population.** Also 12 of 55 events have no minute bars (the April cap-kill class). ⚖ Evidence only; entry discipline is the operator's. |
| **`docs/analysis/delayed_entry_campaign_policies_327_2026-08-30.md`** | **the missed EPs, CAMPAIGN level (Stage 3): 13 pre-registered policies (entry+stop+management+re-entry+abandon), per NAME** | 🔑 **MANAGEMENT is the dominant dimension, and RECALL needs the breakout side.** Same EPC-REC entries, only management varied: **no-management (stop + 20-session hold) +162.3R vs the live shape (+2R partial→BE→trail) +52.1R — the live harvest gives back two-thirds of what the entry finds** (trail −72R, partial −37R, BE ~0; FTNT +24.7 vs +13.3, STX +53.6 vs +12.0, UMC +43.8 vs −0.0, all bar-verified). Every pullback-entry policy enters only 23/43; EPH-BRK-carrying policies enter 41/43 — 18 events (the 04-08 cluster) are reachable ONLY via the breakout side. Re-entry is small and bounded: best = re-enter on EP-high strength proof (+12.9R net, −2.5R failed-attempt cost); unlimited re-entry doubles cost for no added win. Events ≥4R net: no-mgmt 10, live-shape 5. Bleed proxy (stopped-44): M-none is the only management arm positive on BOTH cohorts; composites need ~8–18% of fires to be real tails vs a measured ~2.3% raw-screen base rate — **selection, not campaign design, is the binding problem (P13)**. ⚠ Outcome-conditioned label inflates everything, maximally flattering no-management; #270's Layer-3 found the opposite sign on its cohort (P8). Proposes persisting the ex-ante screen daily to build the first non-conditioned population. |
| **`docs/analysis/327_stage0_anticipation_diagnosis_2026-08-30.md`** | **the #270 machine itself: all 19 `mi_anticipation_lifecycle` rows, `mi_job_runs`, gate volumes 05-01..08-29** | 🔑 **STAGE 0 VERDICT: REBUILD the watch lane — a re-seed does not suffice.** The machine ran for EXACTLY ONE evaluation cycle ever (readiness once 06-16 17:35 ET; the 3b entry job once at 16:20 ET — 75 min BEFORE the table's first-ever write, so the entry layer has examined zero names) and was paused 3h later; that alone explains the zero settled outcomes. Seed admits ~3.7/session raw, ~1.1/session confirmed vs the EP scan's 39 (n=3,266/83 sessions) — 3% of the population, and a different one. `entry_tactic` NULL everywhere because the entry layer was never reached (not a silent failure). The one `ready` row is MNTS, written 5 days stale; its own tactics would have fired 06-12 and LOST. The one-block uncomment still compiles but is unsafe (phantom universe + loud alerts + stale-armed mistimed entries + retroactive hindsight outcomes). 13 defects ledgered D1-D13; salvage = harvest core/abstain rule/plumbing; `replay()` + the table stay frozen per ADR 0013. |
...
| **`docs/analysis/327_delayed_entry_tail_read_2026-09-22.md`** | **the LIVE, non-outcome-conditioned lane: all 3,537 fires (115 near-zero-stop excluded), 2,980 settled at 20 sessions, 2026-08-25 → 09-21** | 🔑 **ALL FOUR PATTERNS ARE NEGATIVE AND THE TAIL IS MOSTLY A TIGHT-STOP ARTIFACT.** `ep_close_620_prox` −0.581R (n=1021) · `ep_close_reclaim` −0.635R (n=769) · `ep_high_break` −0.575R (n=181) · `ep_low_reclaim` −0.605R (n=1009); −1,797R pooled. **30 of 2,980 reach 3R = 1.01% against the 13% #624 records as healthy.** Raising the stop-width floor collapses the tail — 0.66% at 1%, 0.47% at 2%, 0.24% at 3% — and four of the six biggest winners disappear at 2% (RCT +9.58R on a 0.62% stop, BZAI, RGNX, CALC); the mean improves at NO floor. 🔑 **THE NO-TARGET ARM IS −1.00R WITH ZERO POSITIVES IN ALL 2,980 ROWS, every pattern: absent a target, every fire eventually reaches its stop — so the measured defect is STOP PLACEMENT, not entry timing, which routes to #545 rather than to retiring the lane.** ⚠ This is the FIRST non-outcome-conditioned read of this lane — it does not carry the `delayed_entry_stage2`/`campaign_policies` inflation, and it disagrees with them in sign, which is what a non-conditioned population was expected to do. ⚠ 557 fires (16%) are unsettled with NULL outcome columns — excluded, never counted as losses; they are young (5.7-8.5 of 20 sessions), which is what defuses the 'settled rows are the losers by construction' trap. ⚖ Graduation is the operator's call, in the PLAN standing-ask table. |
...
1. ~~The whole ladder has only ever been priced on names we ENTERED AND WERE STOPPED OUT OF.~~ **ANSWERED 2026-08-30 (Stage 2, `delayed_entry_stage2_327_2026-08-30.md`): the ranking inverts on the missed population** — but 12 of 55 events are dark (no minute bars: the April cap-kill class) and the label is outcome-conditioned, so what remains open is (a) the dark 12 after a backfill extension, and (b) the live-blend question: no ex-ante feature measured yet separates a real-EP fire from a stopped-cohort-shaped bleed at trigger time (P13).
