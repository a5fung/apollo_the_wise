...
- **✅ FIXED 2026-09-08 (was an accepted limitation, and it cost real money before it was).** The staleness gate used `last_trading_day(today − 1)`, which is weekend-aware but **holiday-blind**, so the trading day after a market holiday demanded a regime row that could never exist. **It fired for real on 2026-09-08, the Tuesday after Labor Day: both books floored to 25%, and two live entries (PHVS, SEI) took $12.35 of risk each against the ~$37 that Choppy's own 0.75 multiplier allowed.** The feed was healthy throughout — Friday 09-04's row was the correct most-recent one. **The fix reads the last session we actually HOLD closes for** (`_last_ingested_session`, `max(trade_date)` from `mi_daily_closes`): a holiday produces no rows, so the table carries the trading calendar without anyone maintaining one. Not circular — freshness is judged from the CLOSES table, never from the regime table itself. The old calendar rule remains the **fallback** when that read is unavailable, because flooring is the safe direction when we cannot tell. Pinned by `tests/test_regime_freshness_holiday.py`, including that a genuinely failed nightly still floors.
- **🔒 HARDENED 2026-09-10 (#636, operator-signed) — the fix above was correct only because of another job's schedule.** `_last_ingested_session` asked for `max(trade_date) ... WHERE trade_date <= today`. Today's closes land ~17:52–18:00 ET, so at 9:31 no row for today exists and the query correctly answers *yesterday*. **But that made the whole sizing path depend on the closes job staying late:** if a row dated today ever existed before the bell — the ingest rescheduled, a backfill, a manual re-run, a future intraday writer — the threshold would become **today**, every regime row dated yesterday would read STALE, and **every entry would be floored to a quarter size, every day, until someone noticed** — the identical failure this section documents, re-entered through a door nobody was watching and triggered by a change to a job with nothing to do with sizing. **The comparison is now strictly `<`**: behaviour-identical today, and the coupling is gone. Pinned by `tests/test_regime_freshness_same_day_row_636.py`, which asserts both that the ordinary answer is unchanged and that a same-day closes row cannot move the threshold (RED-proven: reverting to `<=` fails it). Classified a **bug fix, not a criteria change** — no threshold, multiplier or admission rule moved — so no backtest gate; operator sign-off was still required and given, because it is the sizing path.
...
**Known limitations carried forward** (see "Position sizing" section above for detail): staleness gate is now holiday-aware via the last ingested session (fixed 2026-09-08; the calendar rule survives as the fallback); the fail-loud dedup has a bounded race under the ORB monitor's 5-way concurrency (a handful of duplicate Telegrams possible on the first fallback morning, never a sizing-correctness issue); the offline `backtester/engine.py` / `backtester/tracker.py` sizing copies are UNCHANGED (not one of the 3 identified sizing sites) and will diverge from live sizing if/when used for future regime-replay evidence work.
