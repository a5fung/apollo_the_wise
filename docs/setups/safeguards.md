...
7. **`drawdown_breaker`** (`BLOCK_DRAWDOWN_BREAKER`) — ACTIVE as of 2026-06-03. **EFFECTIVENESS REVIEWED 2026-07-30 (`data_gated_reviews::drawdown_breaker_active_effectiveness`, resolved): NET-HELPED, KEEP AS-IS — no threshold change.** One REDUCE trip in the whole active phase (2026-06-05 → 07-06, 31 days); BLOCK has never fired and zero entries carry `block:drawdown_breaker` lifetime. Enforcement is REAL: paper REDUCE-window trades ran avg risk $297 / notional $7,311 vs $917 / $18,504 outside — ~1/3 of normal, the 0.5× compounding with regime sizing. Those 6 trades lost $2,493 at reduced size vs roughly 3× that at full. **The tiered design's explicit risk — missing recovery-winners — did NOT materialise (1 winner in 6; there were none to miss).** ⚠ Limit: the comparison group is a single trade, so the saving is directional not measured; the untested case is a REDUCE window that coincides with a real winner cohort. Persisted state machine; when `mi_safeguard_state.state='TRIPPED'`, blocks. See "Drawdown breaker — Mechanics" below.

## Position sizing — regime-keyed risk multiplier (#456, operator-ruled 2026-07-26)

**Phase**: code SHIPPED behind a feature flag, **flag OFF by default** — today's exact VIX-scaled + `qqq_ema_bullish`-halve behavior is unchanged in production until the operator flips it. This section documents the risk_pct COMPUTATION at the spec-builder step (upstream of the safeguards list above); the drawdown-tier + per-strategy composition below (`final_shares = ...`) is unchanged and multiplies on top of whatever risk_pct this section produces.
...
2. `order_manager.prepare_9m_day2_orb_order` (9M Day2 ORB entry, real money).
...
- **`today` is THREADED, not independently re-read** (advisor-caught during review): `prepare_orb_order` / `prepare_9m_day2_orb_order` take an explicit `today` param — the caller's ALREADY-resolved value (`process_new_alerts_live`'s `today`, `submit_9m_day2_trade`'s `today = et_today()`), the same value used for the regime fetch + alerts query + `submit_trade_entry(today=...)` — passed through the `_magna_spec_builder` / `_ninem_spec_builder` closures in `live_tracker.py`. A second, independent `et_today()` call inside `order_manager.py` would have been an unpinned second clock source in the money path (a real risk under `EXECUTION_MODE=http`'s cross-container split, or a bar-fetch retry spanning a midnight-ET boundary). Both functions default `today=None → et_today()` only for callers without one on hand. Pinned in `tests/test_regime_sizing.py`.
