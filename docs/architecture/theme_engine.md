...
- **Nightly THEME-HIERARCHY health check (#506, 2026-09-26, `health_checks.run_theme_hierarchy_health_check`, wired into `_post_nightly_audit_job` at 17:30 ET — after the 17:00 engine, right after `run_ecosystem_reactivation_check`, so tonight's board + ecosystem mappings exist)**: WHY — `parent_theme` sat at 1-of-94 for 11 days after the #471 arm flipped and NOTHING could have told anyone (four internal engine files write it, zero operator surfaces read it); the operator noticed unprompted. **Reads through #505's resolver (`theme_ecosystems.containment_parent` / `resolve_theme_parent`), never re-derives the overload split** — same-day landing, cross-referenced in both commits. Four metrics, operator's own words 2026-07-27, each threshold DERIVED off real `mi_themes` history at RUNTIME every run — never a picked constant, so the floor keeps re-calibrating through whatever the eventual `theme_parent_pass` flip does to the population instead of needing a follow-up edit:
  1. **Share with a real (containment) parent** — live themes (mirrors `get_active_themes`'s own 7-day/non-Retired definition) that are `resolve_theme_parent`'s "child" kind. Measured 2026-09-26: 8.5% (10/118) — EXPECTED to stay low until the operator flips `theme_parent_pass` ON (THE LINE: CHANGE_PROCESS + sign-off on #505's dry run first; today's 10 links predate the pass, from ADR-0025 Route A, fired twice lifetime). Pages on a day-over-day DROP in this share past the historical max daily move + margin (`_derive_orphan_growth_alert_pp`, replayed off the last ~60 distinct `theme_date`s), never on the level — a known, already-tracked condition would otherwise page every night, and the flip can only RAISE this share.
  ...
  3. **Catch-all size** — `resolve_theme_parent`'s "catch_all" kind (E-UNASSIGNED/unmapped) as a share of the live board (2026-09-26: 3/118, 2.5% — healthy, per ruling (5) that the ecosystem is the default parent). Operator: "when catch all grow too big, that's a red flag itself" — pages on GROWTH.
  ...
  Metrics 1-2 derive LIVE off `mi_themes`' own history every run. Metrics 3-4 CANNOT: `mi_theme_ecosystems` is current-state-only (no date column) — they self-baseline against this check's OWN accruing `theme_hierarchy_health` audit rows and stay silent until enough self-collected history exists (n=0 at ship, same bootstrap idiom as `run_db_growth_check`). The evening line and audit summary render ruling-(5)'s own vocabulary — "N nested under a real theme · M root under their ecosystem · K in the catch-all" — never "orphan," so the wording cannot contradict the SSoT's "117 of 118 have a parent" claim (a root under its ecosystem IS a parent, by that ruling). One such line renders on the operator's evening briefing EVERY night (`get_theme_hierarchy_evening_line`, reads tonight's own audit row, fails silent) — the actual gap the 1-of-94 incident exposed, since no surface rendered this at all before #505/#506. Skips (records nothing, computes nothing) below `_HIER_MIN_LIVE_THEMES=10` live themes (the 07-28 zero-row crash night). Tests: `tests/test_506_theme_hierarchy_health.py`.
