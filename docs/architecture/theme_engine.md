...
- **#505 PARENT PASS — parent-child holds on EVERY discovery path (built 2026-09-26, DARK behind `theme_parent_pass` in `mi_safeguard_state`, fail-closed OFF; operator 2026-07-27: *"parent child relationship must work regardless on how themes are discovered, if not then it's broken"*)**. Design of record = ruling (5) 2026-07-27 (`docs/design/theme_system_consolidation_2026-07-27.md` §5): **a theme is either a CHILD of a real theme or a ROOT directly under its ecosystem; E-UNASSIGNED is the catch-all** — no synthetic placeholder themes. So "every theme has a parent" is satisfied by the resolver, and the pass only adds CHILD links. Three pieces, one commit:
  ...
  - **The pass (`theme_engine._run_parent_pass`, wired in `run_theme_engine` AFTER the auto-retire block and Arm B, BEFORE `_restore_sub_theme_links`)** — runs over the FINAL board so it is path-agnostic by construction: it never asks how a theme was born. Every childless live theme (non-Retired, non-Fading, mapped to a real ecosystem) is offered its ONE best same-ecosystem BROADER candidate to `theme_merge_arm.adjudicate_containment_pair` — the pass's OWN adjudicator (Sonnet tier via `THEME_PARENT_ADJUDICATION_MODEL`), asking CONTAINMENT ("does the child sit inside the parent?"), never Arm B's same-catalyst MERGE question. The trigger `propose_parent_candidates` is PURE (no DB/LLM) and shared with the dry-run probe, so the probe prints exactly what an armed night asks. **Why thesis adjudication and not ticker containment**: measured 2026-09-26 before building — ticker containment (Route A's ≥0.8 / ≥3-shared trigger) fires on **1 of ~6,800 board pairs**; the 10 links that exist are ticker-DISJOINT thesis relationships from Arm B. Ticker overlap is therefore a PRIORITY signal (shared tickers → shared ≥5-char name tokens → parent breadth), never the gate.

    **🔴 2026-09-26 — the FIRST build asked the wrong question, corrected same day, dark the whole time.** The pass launched dark reusing Arm B's `adjudicate_merge_pair` (parent = A, child = B, "same DRIVER?"). A paid preview over the full 37-item queue (`scripts/probes/_505_parent_pass_preview_2026-09-26.txt`, log_spend=False, never armed) came back **3 PARENT_CHILD / 5 MERGE / 29 DISTINCT** — the merge adjudicator answered DISTINCT on pairs that ARE containment (e.g. `'Pure-Play NAND/DRAM Memory Chip Makers'` vs `'AI-Driven Memory & Storage Supply Shortage'`: different demand catalysts is the right answer to "same driver?", the wrong answer to "does one sit inside the other?"), and its own reasoning sometimes mislabelled which theme was A and which was B. Operator-approved fix: a **dedicated containment adjudicator**, `adjudicate_containment_pair(child, parent)` (`theme_merge_arm.py`), on the **Sonnet tier**, run the SAME day before anything armed — this pass never adjudicated a single real pair on the old question in production (toggle `theme_parent_pass` was OFF throughout).
    - **Prompt** labels the two themes by ROLE — `CANDIDATE CHILD: {name}` / `CANDIDATE PARENT: {name}`, each with its description and member tickers — **never A/B** (the exact ambiguity that produced the mislabelled reasoning above). One question: is the CHILD a narrower sub-theme riding a sub-driver of the PARENT's thesis?
    - **Verdict → action**:
      | Verdict | Meaning | Action |
      |---|---|---|
      | `CHILD_OF` | child sits inside parent | `parent_theme` set + `sub_theme_parents` map (`_restore_sub_theme_links` re-sets it against the truly final list); `theme_parent_pass_linked` |
      | `INVERTED` | labels were backwards for this pair — the candidate PARENT actually sits inside the candidate CHILD | NO link; `PARENT_PASS_INVERTED_COOLDOWN_DAYS` = 7d cooldown (shorter than PEERS/UNRELATED's 30d — our call: a real containment relationship exists, membership drift re-broadening one side is worth re-checking sooner than a genuine non-relationship, never zero so a stable pair isn't re-asked nightly for free); `theme_parent_pass_inverted` |
      | `PEERS` | related, same level, neither nests in the other | NO link; 30d cooldown (`MERGE_DISTINCT_COOLDOWN_DAYS`, the SAME `mi_theme_merge_cooldowns` table and duration the old DISTINCT verdict wrote — the verdict label itself is carried through as `PEERS`, not rewritten to `DISTINCT`, and `get_merge_distinct_pairs` doesn't care what the verdict string says, only that the pair has a live row); `theme_parent_pass_distinct` |
      | `UNRELATED` | no relationship at all | same as PEERS: NO link, 30d cooldown, `theme_parent_pass_distinct` |
      | `ERROR` / raised | adjudication failed | audited (`theme_parent_pass_error`), no cooldown — re-asked next night |
    - **Model**: `THEME_PARENT_ADJUDICATION_MODEL` — a NEW `RESOLVED_ROLES` entry (tier `sonnet`), never a hardcoded id; resolves today to `claude-sonnet-5` in production (same cache the other sonnet roles read — verified live via `mi_model_resolution`), falls back to the `claude-sonnet-4-6` pin offline. `thinking=DISABLED` (`shared/llm_thinking.THINKING_DISABLED`) — Sonnet's unset-thinking default SHARES `max_tokens` with the tool output (#575), and `analysis_scratchpad` already IS the reasoning surface. Spend logs under a NEW caller name, `theme_parent_adjudication` (`shared/output_ceilings.py`, ceiling 1000, sized by analogy to `theme_merge_adjudication`'s measured 700 — no sample of its own yet).
    - Cap `PARENT_PASS_CAP_PER_NIGHT` = 6 Sonnet calls (~$0.01-0.02/night, was ~$0.002 each on Haiku). **Arm B gets first right of refusal**: a child whose BEST candidate pair is one Arm B's uncapped Stage-A pairing would propose is DEFERRED, not offered its second-best — the dry run showed the one ticker-contained pair on the board (`Small Satellite & Space Services Rotation` inside `Space Economy: Satellite Communications & Launch Services`, containment 1.00, both `defense_aero` stems) is exactly such a pair, and the fallback candidate shared nothing with the child. Arm B's own verdict then unblocks it (DISTINCT → cooldown → next candidate; MERGE → gone; PARENT_CHILD → parented) — Arm B's own vocabulary, completely unchanged by this fix. Fail-open per pair and for the whole pass (the 2026-07-28 lesson — a crash is not a parity failure). One `theme_parent_pass_ran` heartbeat per armed run with counts.
  - **Coverage by path, stated plainly**: Lane-1 discovery, the Route-B split, `/promotetheme` and every incumbent are on `all_themes` tonight → considered tonight. `shadow_promoted` rows are written by scheduler step 5d AFTER `run_theme_engine`, and tonight's NEWBORNS have no ecosystem row until `_map_ecosystems_nonfatal` runs after `_save_themes` → both are considered the NEXT night as incumbents (one night late, never never). The dry run found 8 `shadow_promoted` themes on the board, 5 of them in the WHOLE 37-item ask queue — none of tonight's 6 capped asks (the queue drains at 6/night, so a queue member isn't necessarily asked tonight; see the probe's `shadow_promoted_split`).
  ...
  - **Dry run / backfill = `scripts/probes/_505_parent_pass_dry_run.py`** (read-only, $0; output `_505_parent_pass_dry_run_out_2026-09-26.txt`): resolved parent per theme, tonight's capped asks + the whole queue with why, the 5 biggest families today and as an upper bound, and the backfill priced as ONE number via `pricing_for`. ⚠ **APPROXIMATION, not an exact replay of an armed night** (found on adversarial review 2026-09-26 — the probe originally claimed "the SAME inputs"; it does not): the probe reads YESTERDAY's already-saved `mi_themes` snapshot, not tonight's final post-engine board, and its sectors come from one `mi_stock_scores` join (top ~300 by rank) — the real engine's `stocks_by_ticker` is the top-`ASSIGN_POOL_CEILING` RS leaders PLUS every existing theme ticker outside that list, backfilled via `get_rs_for_tickers` + the persistent `mi_ticker_overrides` sector cache. Close enough to sign off the design, not identical to a real run. `--adjudicate` calls the SAME `adjudicate_containment_pair` an armed night calls (log_spend=False) but **writes nothing**: no `parent_theme` link, no cooldown row, no DB mutation — its printed verdicts and tally are exactly what an armed night would produce. Links are written only by the armed nightly pass once `theme_parent_pass` is ON. OPERATOR-authorised only. ⚖ **THE LINE**: theme structure feeds the judge's theme axis and allocation → the flip is CHANGE_PROCESS + operator sign-off on the dry run, and the toggle stays OFF until then. Forks for that sign-off: (a) drain the queue via the nightly cap (~7 armed nights, the toggle does the writing) vs a one-shot preview-then-manual-backfill session using `--adjudicate`'s printed verdicts; (b) ask zero-signal pairs at all (a childless theme is offered the broadest same-ecosystem theme even with no shared ticker/token — Arm B's own sector-family anchor pairing is the same heuristic and produced real links like `Non-Alcoholic Beverages & Energy Drinks → Defensive Consumer Staples Rotation`; the cost is one no-containment cooldown per wrong ask) vs require ≥1 signal. Tests: `tests/test_505_parent_pass.py` (29) + `tests/test_theme_merge_arm.py`'s containment-adjudicator section (tool schema, prompt labelling, spend/model/thinking pins) + `tests/test_505_probe_dry_run.py` (probe wiring).
...
- **Nightly THEME-HIERARCHY health check (#506, built 2026-09-26, hardened same day on adversarial review, `health_checks.run_theme_hierarchy_health_check`, wired into `_post_nightly_audit_job` at 17:30 ET — after the 17:00 engine, right after `run_ecosystem_reactivation_check`, so tonight's board + ecosystem mappings exist)**: WHY — `parent_theme` sat at 1-of-94 for 11 days after the #471 arm flipped and NOTHING could have told anyone (four internal engine files write it, zero operator surfaces read it); the operator noticed unprompted. **Reads through #505's resolver (`theme_ecosystems.containment_parent` / `resolve_theme_parent`), never re-derives the overload split** — same-day landing, cross-referenced in both commits. Five metrics, operator's own words 2026-07-27, each threshold DERIVED off real history at RUNTIME every run — never a picked constant, so the floor keeps re-calibrating through whatever the eventual `theme_parent_pass` flip does to the population instead of needing a follow-up edit:
  1. **Share with a real (containment) parent** — live themes (mirrors `get_active_themes`'s own 7-day/non-Retired definition) that are `resolve_theme_parent`'s "child" kind. Measured 2026-09-26: 8.5% (10/118) — EXPECTED to stay low until the operator flips `theme_parent_pass` ON (THE LINE: CHANGE_PROCESS + sign-off on #505's dry run first; today's 10 links predate the pass, from ADR-0025 Route A, fired twice lifetime). Pages on a day-over-day DROP in this share past the historical max daily move + margin (`_derive_orphan_growth_alert_pp`, replayed off the last ~60 distinct `theme_date`s — see the pre-#471-fix exclusion below), never on the level — a known, already-tracked condition would otherwise page every night, and the flip can only RAISE this share.
  2. **Lost-parent-link** — a theme that carried a real containment parent on an earlier day and lost it on a LATER day without itself retiring — the operator's "half-completed nesting," and the `_canonicalize_theme_names` / `_restore_sub_theme_links` known gap actually firing. Two framings were tried and rejected FIRST, both measured 2026-09-26: a parent-centric "parent with zero children" (all 4 real candidates were single-child parents whose one child retired NATURALLY — attrition, not breakage), and a bare "child lost its link" scan (16 events/~90 days, but its biggest cluster — 3 in one day — was the PARENT itself retiring/renaming that night, the engine correctly clearing a dangling pointer, not the bug). **The shipped check also verifies the lost parent NAME is still a live theme** before counting a loss — that is what actually discriminates the bug from the engine working as designed. Pages when the trailing-7-day count of genuine losses exceeds the historical max (`_derive_lost_link_alert`).
  3. **Catch-all size** — `resolve_theme_parent`'s "catch_all" kind (`theme_ecosystems.E_UNASSIGNED`/unmapped) as a share of the live board (2026-09-26: 3/118, 2.5% — healthy, per ruling (5) that the ecosystem is the default parent). Operator: "when catch all grow too big, that's a red flag itself" — pages on GROWTH. ⚠ Comes from `_hier_ecosystem_breakdown`, which counts by `eco_map` alone — it INCLUDES a theme that is already a CHILD of a real parent but whose ecosystem happens to be `E_UNASSIGNED`; it is a different count from metric 1's child/root/catch_all split (`_hier_parent_kind_counts`), reported separately on purpose (both land in the same audit row).
  4. **Ecosystem concentration drift** — one `e_code`'s share of the live board swinging while others empty, usually classifier drift not a real market move (2026-09-26: 21 buckets, top E-SAAS 12/118, 10.2%). Two-sided (growth or hollowing-out), unlike 1-3.
  5. **Childless nominated parents** (added 2026-09-26, adversarial review) — the operator's OWN "nominated parents with no children" framing (distinct from metric 2's "had a link, lost it"), made measurable: a PARENT_CHILD adjudicator verdict — Route A `theme_subtheme_routed`, Arm B `theme_merge_parent_child`, or #505's own `theme_parent_pass_linked` (`db.PARENT_NOMINATION_EVENT_TYPES`, one shared regex parses the nominated parent name out of all three summaries) — named a parent in the last 30 days that is STILL a live theme but has no live theme pointing at it as a child today. A nominee that later retired is attrition, not a bug, and is excluded the same way metric 2 already excludes it.

  **Ecosystem-read failure is a graceful DEGRADE, never a false alarm** (adversarial-review fix, 2026-09-26): metrics 1-2 don't need `eco_map` at all (`containment_parent` reads no ecosystem data); if `get_theme_ecosystem_map` fails, metrics 3-4 and the root-vs-catch-all half of metric 1's kind-count are skipped (recorded `None`, `ecosystem_skipped_reason` set in the audit row) rather than computed off an EMPTY map — an empty map makes every live theme resolve to `E_UNASSIGNED`, so a naive continue would report a false 100% catch-all / 100% concentration off a pure read failure and could page against real history.

  **A genuine internal failure is LOUD, never dark** (adversarial-review fix, 2026-09-26): the check used to return early on a failed window query with NO audit row and NO alert — the function never raises, so the scheduler's own wrapping `except Exception: notify_job_failure(...)` around this call was unreachable, and the evening line silently vanished with no trace. Any such failure now writes a `status=error` audit row AND calls `core.notifications.notify_job_failure("theme_hierarchy_health_check", ...)` directly (`health_checks._hier_report_failure` — the same "a health guard calls its own notifier" idiom `check_recorder_failure_rate` already uses in this file), while the function itself still never raises. **Registered in the nightly liveness sweep** (`_DETECTOR_LIVENESS_TABLES`, keyed on `mi_audit_log` + `event_type = 'theme_hierarchy_health'` via its `extra_where` slot) so a night this check never even RAN (crash before reaching its own audit write, or dropped from the scheduler) is caught too — the exact "1-of-94 for 11 days, nothing could tell anyone" failure mode this whole check exists for, reproduced one layer up. ⚠ Follow-on caught the same night, before shipping: that loud `status=error` row (and the pre-existing thin-board `skipped` row) writes `metrics={}` — a real DICT, so a bare `baseline is not None` check in `_evaluate_theme_hierarchy` passed while every field was missing, and `baseline.get(x) or 0.0` then substituted 0 for the missing value, manufacturing a false day-over-day SPIKE the very next night off nothing but a read/skip failure. Every self-baselined check (metrics 3-5) now also requires `baseline.get(<field>) is not None`, matching the orphan-growth branch's already-correct guard.
