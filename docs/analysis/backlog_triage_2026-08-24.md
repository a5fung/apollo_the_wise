...
| **#212** | Questioner/investigator loop for sourcing | CLOSE | **Dropped, not re-homed.** All that exists is a throwaway read-only prototype from June (`scripts/probes/proto_dialogic_dossier.py`, its own header says no DB write, no scheduler), untouched since 06-07, no DoD. Refile from a real case. |
