⛔ **RETRACTED 2026-09-09, hours after it was written — by the operator, in one line:
*"we literally updated rule a few days ago, you ask to change again???? knee jerk"*.**

**He is right and the defect is fatal to the conclusion, not cosmetic.** The MAGNA53 exit rule was
flipped LIVE on **2026-09-06** (commit `b52fdcbc`: partial moves to **+8R**, price-armed breakeven
on at **+3R**). Of the 28 live trades this analysis rests on, **27 ran under the rule that flip
REPLACED and exactly 1 under the current one.**

So the `actual` baseline of +0.01R (Bull) / −0.97R (Correcting) measures **a system that no longer
exists**, and every candidate arm was scored against it. Recommending a change to a rule signed off
three days earlier, on evidence generated by its predecessor, is exactly the knee-jerk he named.

⚠ **This is the ERA error from §2 of `docs/methodology/analysis_standard.md`** — *"a window spanning
a rule change measures a system that never existed"* — which I quoted in the #331 document THIS
MORNING and then walked into by lunchtime. The population check I ran was live-vs-paper and
strategy-vs-deprecated; I never asked which EXIT ERA the rows belonged to, which is the one that
mattered.

**What survives:** the descriptive regime split (Correcting reaches +2R at 43% and converted 0 of 7)
is a fact about the OLD rule and is still worth knowing — it is part of WHY the +8R/+3R flip
happened. **What does not survive: every comparison to `actual`, the whole candidate grid, and the
recommendation.** The standing ask has been withdrawn. Nothing was changed at any point.
