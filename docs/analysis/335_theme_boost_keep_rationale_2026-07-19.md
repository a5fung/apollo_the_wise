...
## Pointers
Shadow table `mi_theme_axis_shadow`; deterministic writer `theme_axis_shadow.log_theme_axis_shadow`; boost math `meta_rubric_compose.compose_final_tier` + `compute_theme_axis_credit`; toggle `composite_authority` (dark). **Cluster (bidirectionally linked — every node points here, this doc points to all):** #335 (this task) · #486 (judge↔theme-engine cross-validation — the near-term payoff) · #329 (composite parent) · #322 (judge→radar feed, deployed) · #467 (coverage loop — **CLOSED verified-live 7/16**, commit d8c57d9; its S1 widened-gate fix is precisely why MODERATE instrumentation just began — the lone MODERATE row) · #235 (gap-discovery, closed 7/19) · the auto-surfacing review `theme_axis_boost_reeval` (data_gated_reviews.yaml) · memory `shadow-zero-effect-check-instrumentation`. Prior docs: `m1b_regrade_2026-07-13.md`, `ep_theme_coverage_loop_design_2026-07-13.md`, `m1_sitting_pack_2026-07-18.md`, `theme_membership_investigation_2026-07-13.md`, `m1_rubric_amendment_draft_2026-07-08.md`.
