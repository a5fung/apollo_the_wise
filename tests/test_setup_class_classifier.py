...
Two layers:
  ...
  2. `compute_setup_class_fields` — the async assembler: reuses structure_axis_shadow's
     `stage2`, calls the 3 new as-of DB primitives, and is independently fail-soft per lookup
     (mirrors the axis-shadow "never guess" discipline).
...
def test_assembler_pulls_threaded_fields_from_r():
    ...
    r = {
        ...
        "week52_high": 60.0, "upgrades_30d": 2,
    }
...
def test_assembler_one_lookup_failing_does_not_blank_the_others(monkeypatch):
    """A sugar-baby query hiccup must not stop the 9M/stage2/ADV fields from still resolving —
    each lookup is independently guarded."""
    pool, conn = make_mock_pool()
