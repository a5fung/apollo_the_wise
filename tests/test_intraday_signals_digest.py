...
def test_empty_detectors_omitted():
    per = [
        ("🎯", "Flag-break", [("AAPL", False), ("NVDA", True)]),
        ("📉", "MA-pullback", []),            # omitted
        ("🛡", "Support-test", [("MSFT", False)]),
    ]
    ...
    assert "🍬NVDA" in msg                    # cohort marker
    assert "AAPL" in msg and "🍬AAPL" not in msg  # non-cohort unmarked
...
def test_long_list_capped():
    rows = [(f"T{i}", False) for i in range(20)]
...
def test_no_entries_disclaimer():
    msg = _build_intraday_signals_digest(D, [("🎯", "Flag-break", [("X", False)])])
