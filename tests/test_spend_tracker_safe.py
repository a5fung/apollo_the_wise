...
def test_raising_log_anthropic_call_is_swallowed_and_warned(monkeypatch, caplog):
    ...
    async def _boom(*, model, caller, response):
    ...
    with caplog.at_level(logging.WARNING, logger=spend_tracker.logger.name):
        ...
        result = _run(spend_tracker.log_anthropic_call_safe(
            model="claude-sonnet-4-6", caller="my_caller", response=object(),
        ))
...
def test_healthy_call_forwards_args_and_logs_no_warning(monkeypatch, caplog):
    ...
    async def _capture(*, model, caller, response):
        calls.append({"model": model, "caller": caller, "response": response})
        return 0.042
    ...
    with caplog.at_level(logging.WARNING, logger=spend_tracker.logger.name):
        result = _run(spend_tracker.log_anthropic_call_safe(
            model="claude-opus-4-8", caller="theme_advisor_x", response=sentinel_response,
        ))
    ...
    assert calls[0] == {
        ...
        # #543: the RAW response travels whole — usage and stop_reason are derived inside
        # log_anthropic_call, so the wrapper can no longer drop one and keep the other (the
        # ten-day theme_assignment outage shape).
...
def test_end_to_end_healthy_call_still_writes_the_api_usage_row(monkeypatch):
    ...
    _run(spend_tracker.log_anthropic_call_safe(
        model="claude-sonnet-4-6", caller="e2e_check", response=_Resp(),
    ))

    assert len(inserts) == 1
    model, caller, in_tok, out_tok, _cc, _cr, cost, sr = inserts[0]
    assert model == "claude-sonnet-4-6"
    assert caller == "e2e_check"
    assert in_tok == 1000 and out_tok == 200
    assert cost > 0
    # #543: stop_reason rode along inside the same response object — no second kwarg to forget.
    assert sr == "end_turn"
