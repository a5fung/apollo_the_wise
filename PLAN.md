...
- #299 | 2026-06-19 | in_progress | P2 tape-features. SLICE A (judge payload structure) + SLICE B (compute core: compute_or_atr/compute_pm_vol_curve/compute_liquidity_tag/build_tape) shipped. EVAL RIG BUILT + machinery-VERIFIED 6/17 (scripts/eval_tape_judge.py, with-vs-without delta surface + judge noise floor + point-in-time minute-bar reconstruction; 8 tz/lookahead tests; doc docs/analysis/tape_judge_eval_299_2026-06-17.md). BINDING FINDING (zero-cost timing histogram, n=569 HIGH): OR÷ATR available AND ORB-tradeable for 79.1% of HIGHs (NOT killed by timing, contra hypothesis); judge gates entry synchronously (ep_detector ~L2489) so tape shifts tradeable selection. SMOKE PASS (5 rows: OR÷ATR 5/5 present, machinery clean, 0 deltas as expected). REMAINING (OPERATOR-TRIGGERED — cost + ADR 0011 self-cert gate): run full `eval_tape_judge.py --days 95 --limit 0 --replicates 3` → operator labels deltas → if majority-correct, CHANGE_PROCESS + sign-off → wire tape into live _judge_shadow
