...
- #266 | 2026-06-19 | in_progress | theme membership validation at BIRTH + identity-change. EVIDENCE RUN DONE 6/17 (SUPPORTED — strip-latency median 6.0d, 73% land >3d after birth, 234 themes carry bad-birth members below the round-trip alarm; ALL removal reasons = description-mismatch = birth-detectable). ROOT-CAUSE RESOLVED 6/17 (deterministic code trace): theme BIRTH (discovery) never runs _validate_theme_membership — it's called ONLY at theme_engine.py:1754 (Mon/Wed/Fri rescore of EXISTING themes) + :2371 (assign-to-EXISTING); the discovery→persist path (Step 3/4) skips it → founding members first validated 6d later. FIX DESIGNED (doc updated): run the SAME #213-tuned validator on discovered themes' members before _save_themes (changes WHEN not WHAT; theme stripped <2 members simply isn't born). GATED: CHANGE_PROCESS + operator SIGN-OFF (membership-behavior change) before ship; shadow-log option available. Re-run probe after to confirm latency shifts left
