...
- #266 | 2026-06-18 | in_progress | theme membership validation at BIRTH. SHIPPED 6/17 (operator-SIGNED): root-cause was discovery never running _validate_theme_membership (only :1754 Mon/Wed/Fri-rescore + :2371 assign-to-existing). Fix = _validate_new_themes_at_birth runs the SAME #213-tuned validator on discovered themes before _save_themes (after name-inheritance), emits theme_birth_validated; min-survivor guard keeps 2-member themes intact; 5 unit tests + CLAUDE.md SSoT same commit. VERIFY-LIVE 6/18: after the next nightly theme run, re-run scripts/_theme_birth_validation_evidence.py → strip-latency shifts left (≤1h vs 6d median) + theme_birth_validated audit rows appear. FOLLOW-ON (separate): identity-change half (re-validate at rename/description-revision)
