...
- #327 | 2026-06-18 | pending | **#270 Step 3 — consolidation ENTRY-WATCH + shadow settlement + 9M-universe FEED (the critical path for the #326 9M cut-over, lead next).** Wire: confirmed 9M sugar babies (mi_9m_day2_candidates) → Family-A consolidation universe → intraday tightness→expansion entry-watch (reuse #94 flag-break watcher + #270 recorder) → record a shadow consolidation-entry per name → settle realized R (reuse #270 _270_harvest/settle_row/simulate). Output: Day-2 ORB & consolidation entries run PARALLEL on the same 9M names → the directional read that lets #326 decide ~7/7 (PULL EARLIER). ALL SHADOW (no real $). TURNKEY spec in docs/analysis/delayed_ep_composition_270.md. 6/18 is heavy so may carry to 6/19, but it LEADS the consolidation thread
...
- #326 | 2026-07-07 | pending | 9M Day-2 ORB entry → RETIRE for consolidation entries (CUT-OVER DECISION, operator vision). MOVE FAST (operator 6/17: weeks not months; all SHADOW/PAPER = no real $ at risk in the swap). CRITICAL-PATH BUILD (the only real blocker, ~weeks, integration not from-scratch): wire confirmed 9M sugar babies INTO the Family-A consolidation universe + shadow-settle a consolidation-entry per 9M name, so Day-2 ORB & consolidation run PARALLEL on the same names — reuse #270 recorder (LIVE) + #94 flag-break watcher (LIVE) + #270 harvest/settle (BUILT) = #270 Step 3 / #311. DECIDE ~7/7, PULL EARLIER on a clear DIRECTIONAL read (operator judgment, NOT a rigid N) → retire 9m_day2 to shadow (sign-off + CHANGE_PROCESS — that gate stays). COUPLING (only real-$ caution): BLOCKS the 9M Day-2 → live-money promotion (Q3); acceleration is shadow-only. Registry ninem_day2_retire_for_consolidation
