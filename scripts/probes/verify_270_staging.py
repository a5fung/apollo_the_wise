...
Read-MOSTLY: writes ONE synthetic ticker ('ZZSMOKE') into mi_anticipation_lifecycle and
...
async def main():
    ...
    from agents.market_intelligence.db import (
        get_pool, get_anticipation_gap_seeds, get_anticipation_state_map,
        get_anticipation_watch_set, get_anticipation_3b_unsettled,
        get_anticipation_ohlcv, upsert_anticipation_lifecycle,
        get_anticipation_lifecycle_board, upsert_stocks_in_play, get_stocks_in_play,
    )
    ...
    for name, coro in (("state_map", get_anticipation_state_map()),
                       ("watch_set", get_anticipation_watch_set()),
                       ("3b_unsettled", get_anticipation_3b_unsettled())):
        try:
            r = await coro
    ...
    if seeds:
        try:
            s = seeds[0]
            bars = de.db_rows_to_bars(await get_anticipation_ohlcv(s["ticker"], today))
    ...
    try:
        ...
        board = await get_anticipation_lifecycle_board()
    ...
    try:
        ...
        await upsert_stocks_in_play(
            ticker=T, source_detector=SOURCE_ANTICIPATION_REENTRY,
        ...
        sip = await get_stocks_in_play(
            source_detector=SOURCE_ANTICIPATION_REENTRY, active_only=True)
        _check("substrate feed JSONB round-trip",
               any(r["ticker"] == T for r in sip), f"anticipation rows={len(sip)}")
    except Exception as e:
    ...
    try:
        ...
        async with pool.acquire() as conn:
            await conn.execute("DELETE FROM mi_anticipation_lifecycle WHERE ticker=$1", T)
            await conn.execute(
                "DELETE FROM mi_stocks_in_play WHERE ticker=$1 AND source_detector=$2",
                T, SOURCE_ANTICIPATION_REENTRY)
