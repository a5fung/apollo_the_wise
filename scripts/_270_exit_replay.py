...
def build_path(tk, trig_date, post_min_bars):
    ...
    path = [{"o": b["o"], "h": b["h"], "l": b["l"], "c": b["c"],
             "kind": "min", "prior_low": None} for b in post_min_bars]
    dbars = DAILY.get(tk, [])
...
def simulate(entry, init_stop, path, rule, bound):
    ...
    risk = entry - init_stop. Returns (realized_R, captured_pct_of_mfe)."""
