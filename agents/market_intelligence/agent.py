...
class MarketIntelligenceAgent(BaseAgent):
    ...
    async def _handle_watchlist_query(self, request: AgentRequest) -> AgentResponse:
        """`/watch` — the 'stocks to watch' surface (#220, merged with /inplay).

        Two modes (one command):
          /watch       — today's ACTIONABLE tight-range watchlist: TIGHTENING/
                         COILED/TRIGGERED from the latest EOD flag scan, with
                         entry-technique annotations per ticker:
          ...
          /watch all   — the full methodology-wide universe (the old /inplay /
                         mi_stocks_in_play union, ADR 0004). #167/#168 will add
                         the actionability filter that defines the default view.

        (#93 / ADR 0005 §3 + memory/user_tight_range_entry_techniques. The old
        /inplay command was retired 2026-06-06; its mi_stocks_in_play substrate
        is unchanged — only the command surface merged.)
        """
        # #220: /watch all → full universe (old /inplay). Bare /watch = actionable.
        tokens = request.task.lower().split()
        if any(t in ("all", "universe", "inplay") for t in tokens):
            return await self._handle_stocks_in_play_query(request)
