        ...
        for ep in eps:
            if ep.get("score_tier") == "HIGH" and ep["ticker"] not in already_alerted:
                ...
                if within_orb_window:
                    ...
                    logger.info(f"EP {ep['ticker']}: outside ORB window ({now_et.strftime('%H:%M')} ET) — alert sent, no order")  # recovery-clock-ok: a wall-clock TIME-OF-DAY label — when the tick actually happened, which is about NOW by definition. Leave it unpinned. What keeps a stale re-run off the ORB path is the FRESHNESS rule, not this: classify_slot calls sessions_opened_between and marks a slot unrecoverable once any session has opened since it, so a 09:00 slot dies at 09:30 and the only live recovery window is roughly [09:00, 09:30) — where the slot and the real clock both satisfy `hour==9 and minute<45` anyway (ORB_QUIET 09:25-10:05 is belt-and-braces). Pinning would make the text claim a time the scan never ran at, and would drop the safe default if that freshness rule ever loosened. (#672, corrected 2026-09-20.)
...
# #649 (2026-09-12): the SAME check, run at three points, because the bare window is
# longer than the one slot #646 (a1) covered. OKTA lost its stop at 16:02 and the first
# thing that could have noticed was the 21:00 backstop — **about five hours, four of them
# inside extended-hours trading, when the position was tradeable and unprotected.**
#
# ⚠ THE REPAIR HALF IS DELIBERATELY ABSENT. Widening `stop_ack_timeout_watchdog` past
# 16:00 is the obvious fix and it is BARRED: it re-arms at the ORIGINAL `orb_low` with no
# #600 re-protect floor (scheduler.py ~1973), so on a trailed position it would LOWER
# protection — a price change, THE LINE, his call. He ruled on the sibling fork the same
# day: page only, no repair arm. So these slots TELL him inside the window; they place,
# cancel and replace nothing.
#
# WHY THESE THREE TIMES: 17:00 is after the 16:20 post-close refresh has put the overnight
# GTC stop back, so a gap there is real rather than the ordinary 16:15 expiry; 19:00 sits
...
async def _coverage_watch_job(slot: str = "evening"):
    """Broker-truth coverage check for one slot of the bare window (#646 a1, #649).

    Reads BROKER TRUTH per open live position via the signed #527 detector and pages
    if any position holding shares has no resting stop. Always writes the slot's audit
    row, whether or not there is a gap: that row is the job's own liveness evidence
    (see `health_checks.run_evening_verify_heartbeat` for the 21:10 slot), and without
    it a quiet evening and a dead job are the same silence.
    ...
    try:
        ...
        await log_audit_event(
            event,
            f"coverage check [{slot}]: examined {result.get('examined', 0)}, "
        ...
        if not gaps and not failed:
            logger.info(
                f"Coverage check [{slot}]: {result.get('covered', 0)}/"
        ...
        logger.warning(
            f"Coverage check [{slot}]: {len(gaps)} gap(s), {len(failed)} unreadable"
        )
    except Exception as e:
...
async def _sustain_reject_replay_job():
    """Run at 18:13 ET (EOD, after the 18:12 analyst-estimates sibling). #593 (2026-09-03):
    for every net-declined ep_rt_sustain_reject ticker-day, reconstruct the CURRENT-era
    MAGNA53 entry and walk the SAME live exit ladder to ask whether OUR OWN bracket would
    have realized >=4R or any positive return — replacing the +20%-price-move test the #593
    signed condition used (a price move is not a trade outcome; IPST/WETO both closed ~49%
    below the declined level and would have stopped out). RECORD ONLY — no broker calls, no
    live-trade mutation, no admission change, no Telegram (THE LINE; see
    sustain_reject_replay.py docstring). run_sustain_reject_replay never raises; the guard
    here is the last-resort wrapper only."""
    try:
        ...
        logger.info(
            ...
            f"{out['population']} net-declined ticker-day(s) ({out['candidates']} candidate(s) "
    ...
    try:
        ...
        for row in pending:
            ...
            try:
                ...
                # has_direct_source is RECOVERED INSIDE build_judge_payload (2026-09-02) — this
                # job used to do it itself, and a cleanup pass found the other eight callers
                # still passing None, i.e. still told a confident "no direct source" on every
                # row. The recovery needs only grounded_text, which the mirror already has, so
                # no caller has to remember. revenue_stage stays None BY NECESSITY: computed
                # live on earnings day, never persisted, so "not checked" is the truth here.
...
def start_scheduler() -> AsyncIOScheduler:
    ...
    # #593 SUSTAIN-REJECT BRACKET REPLAY — 18:13 ET mon-fri (EOD shadow family, after the
    # 18:12 analyst-estimates sibling; needs today's mi_daily_closes row and day-0 minute
    # bars, both long settled by then). For every net-declined ep_rt_sustain_reject name,
    # walks the CURRENT-era MAGNA53 bracket to ask whether it would have made >=4R or any
    # positive return — pure compute + DB/audit, no broker calls, no admission change, no
    # Telegram (THE LINE; see sustain_reject_replay.py docstring).
    _scheduler.add_job(
    ...
    # #646 (a1) + #649: the same broker-truth check at three points across the bare
    ...
    for _slot, _hh, _mm in (("post_close", 17, 0), ("late", 19, 0), ("evening", 21, 10)):
        _scheduler.add_job(
            audit_wrap(functools.partial(_coverage_watch_job, _slot),
                       f"coverage_watch_{_slot}"),
            CronTrigger(hour=_hh, minute=_mm, day_of_week="mon-fri",
                        timezone="America/New_York"),
            id=f"coverage_watch_{_slot}",
            replace_existing=True,
            misfire_grace_time=900,
        )
