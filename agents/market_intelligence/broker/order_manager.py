...
# Fixed notional for the HTF breakout SHADOW sizing (#356 Phase 3). R-per-share is equity-independent,
# so the edge dataset needs NO live account fetch — a constant basis keeps `shares` comparable across days
# and keeps broker I/O out of the every-5-min #94 scan (advisor 2026-06-28).
HTF_SHADOW_NOTIONAL = 100_000.0
_HTF_MAX_LOSS_PCT = 0.08   # the HTF cap (sourced 5-8%); prefer a tighter stop, flag if even base_low exceeds it


def prepare_htf_breakout_order(*, base_high, base_low, sma_10=None, sma_20=None,
                               regime_record=None, notional=None):
    """#356 Phase 3 — compute the would-be HTF breakout-entry order SHAPE (SHADOW; NEVER submitted).

    Entry = base_high (a stop-limit BUY on the flag-high break). Stop = the TIGHTEST of
    {base_low, sma_10, sma_20} whose loss stays within the 5-8% cap; if none fit, the tightest available
    + a `would_reject_reason`. Sizing is off a FIXED notional (R is equity-independent — no live account
    fetch). The 9m template's REJECT gates (stop-too-wide, sub-1-share) become `would_reject_reason` here:
    the spec is ALWAYS returned, never dropped — the shadow wants the rejects (advisor 2026-06-28).
    Returns (spec, would_reject_reason); would_reject_reason is None on a clean order."""
    from agents.market_intelligence.constants import vix_scaled_risk_pct, RISK_PCT
    notional = float(notional) if notional is not None else HTF_SHADOW_NOTIONAL
    entry = float(base_high)
    would_reject = None
    # candidate stops below entry; a tighter (higher) stop = smaller loss
    ...
    min_risk = entry * 0.02
    if risk_per_share < min_risk:
        risk_per_share = min_risk   # 2% floor (mirrors 9m) — guards a near-zero stop from oversizing
    vix_value = regime_record.get("vix") if regime_record else None
    risk_pct = vix_scaled_risk_pct(vix_value, base_pct=RISK_PCT)
    if regime_record and regime_record.get("qqq_ema_bullish") is False:
        risk_pct *= 0.5
    ...
    spec = {
        ...
        "limit_price": stop_limit_buy_price(entry),
