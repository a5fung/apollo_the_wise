...
| **Execution/Intelligence split + staging** (#256 — service split so detection deploys can't restart trade execution; staging pipeline before live money) | **P0** | **W2 DONE + 🚀 CUTOVER PERFORMED 6/13 (operator-authorized, markets closed) — LIVE ON PAPER as TWO services.** apollo-execution (SERVICE_ROLE=execution, creds+streams, 27 jobs, passed full trade preflight) + apollo-market=intelligence (EXECUTION_MODE=http, no creds, 42 jobs). HTTP transport VALIDATED end-to-end (real position data crossed the wire; 0 ExecutionUnreachable). Partition omission-guard + dry-boot validated per role; ADR 0012 written. Commits d002114·035e0cf·cdb908a·0b48a3c·fb40e38. | **⭐ Mon 6/15 9:31 ET = FIRST LIVE ORB-via-http (#277)** — pre-open check + handoff watch + rollback per `docs/ops/monday_split_golive_playbook.md` (calendar event + session-only cron e2ec6871 set). Then → W3 staging compose + `deploy.sh staging` (needs operator BotFather staging-bot token) → W4 DR/ADR closure | task #256/#277 · plan `…/execution-intelligence-split-256.md` · ADR 0012 |
