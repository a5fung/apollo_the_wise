...
| **North Star** — theme/narrative + CATALYST load-bearing in EP | **P1** | C1 catalyst_type LIVE; #200 theme-gated + #201 fire-panel ADVISORY shipped (verify-live 6/8); catalyst-axis baking (#187 EDGAR✓ · #189 materiality · #190 grounded-Sonnet grade); model-selection BASELINED (#188✓); **#210 catalyst-sourcing BACKBONE stood up in SHADOW 6/7** (Wave A✓ Benzinga corpus+relevance-filter · B✓ provenance KPI+self-verifier · C-Part1✓ Perplexity-boost shadow · D✓ 8-K EX-99 concat; E + #238 dilution-sourcing + demotion/flip gated post-Monday); **OPERATOR DIRECTIVE 6/7: gap-alone-≠-HIGH, real catalyst LOAD-BEARING** (evidence engine `fire_status_r_cohort.py` shipped); **6/9: judge spine W0–W4 BUILT+DORMANT, flip slipped to first alert-bearing day (Mon alert predated deploy, Tue dry — gate terms unchanged, 6/13 holds unless Wed–Thu dry); #251✓ #213✓; Lane-2→judge-theme-axis STAGED ahead of the Wed–Thu plan (semantic narrative matching closes the RCAT new-joiner gap; plan `lane2-judge-theme-axis`); #214 theme-naming trio staged (breadth prompts deployed + inheritance guard + get_active_themes retired-gap root fix); grounded Sonnet-vs-Opus eval RUN (17 rows, operator labels pending — model decision deferred until theme-axis input gap closes)** | **Wed 6/10 verify sweep (calendar 7:30am PDT + pickup checklist)** · theme-as-EP-signal n≥40 ~8/1 · phase5 advisory composite · quarterly model-eval 8/1 (#207) · ⭐ phase6 GATING ~Q4 | memory `north-star…` (Track B) · `phase5/phase6` reviews · `ep-conviction-floor…` · `docs/model_selection_baseline.md` |
...
| **Execution/Intelligence split + staging** (#256 — service split so detection deploys can't restart trade execution; staging pipeline before live money) | **P0** | **SCOPED + W1 STARTED 6/10** (operator-confirmed plan `~/.claude/plans/execution-intelligence-split-256.md`): survey done (broker upward deps infra-only; 44 boundary sites / 3 files), boundary checker shipped report-mode; decisions a–d locked on recommendations (HTTP transport · staging DB = nightly prod-restore · staging bot token · W2 cutover only if landed by ~6/17) | W1 facade + 44-site migration (now) → W2 process split + `EXECUTION_MODE` flag (~6/13–6/17, else post-6/22) → W3 staging compose + `deploy.sh staging` → W4 DR/ADR closure | task #256 · `scripts/check_execution_boundary.py` |
