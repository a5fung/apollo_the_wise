...
| **Apollo v1.1 ? v2.0** (successor program ? `docs/roadmap/apollo-v1.1-v2.0.md`; judge/catalyst/theme work continues HERE) | **P1** | **🏁 NORTH STAR CLOSED 6/11** (one day early; ADR 0011 IMPLEMENTED-LIVE: judge = the live paper grade authority on OPUS 4.8, advisory stack retired, model-checked via operator labels, verified-live on 3 Opus alerts with exact reconcile + 0 fail-opens; #240 + waves all completed; NEVER reopened per locked DoD). v1.1 proposal **APPROVED 6/11**; **🚀 LAUNCH 6/22 locked (#272): docs/roadmap/launch-2026-06-22.md = 10-day runway + airtight launch-DoD; coincides with the live-$ GO/NO-GO** (+ Gemini-review amendments adopted into the doc) | W1 #268 ✓ DONE 6/12 (Phase B 1307/1307: floor +0.90R vs judge +0.95R, promote cohort +1.26R = the edge; kill/scale bands SIGNED into safeguards.md w/ quarterly-review + logged-override conditions) ? W2 entry-mechanics (the #1 expectancy leak) ? W3 exits/harvesting ? W4 #267 chart-vision ? W5 experience seed (#254/#255) ? standing reads: theme-as-EP-signal n?40 ~8/1 ? quarterly model-eval 8/1 (#207, playbook) | roadmap doc ? CHANGELOG 2026-06-11 ? memory `north-star?` ? `docs/model_selection_baseline.md` |
...
| **Execution/Intelligence split + staging** (#256 — service split so detection deploys can't restart trade execution; staging pipeline before live money) | **P0** | **W2 DONE + 🚀 CUTOVER PERFORMED 6/13 (operator-authorized, markets closed) — LIVE ON PAPER as TWO services.** apollo-execution (SERVICE_ROLE=execution, creds+streams, 27 jobs, passed full trade preflight) + apollo-market=intelligence (EXECUTION_MODE=http, no creds, 42 jobs). HTTP transport VALIDATED end-to-end (real position data crossed the wire; 0 ExecutionUnreachable). Partition omission-guard + dry-boot validated per role; ADR 0012 written. Commits d002114·035e0cf·cdb908a·0b48a3c·fb40e38. **HARDENED 6/13: HTTP-timeout split (3cb4b9c) — was a flat-15s Monday-breaker on trigger_orb_entry; #278 (deploy.sh preflight false-fails on creds-less intelligence) + #279 (/simplify deferrals) filed for post-Monday.** **🏁 W3 STAGING BUILT + VALIDATED END-TO-END 6/13 (20c0e50): on-demand mirror (`deploy.sh staging`), label-guarded prod-dump seed, free-mem gate; smoke GREEN first run — 15,279 audit rows seeded, split boot 42/27, `/status`+`/trades` cross http from Telegram, 0 err; torn down clean (prod intact).** | **⭐ Mon 6/15 9:31 ET = FIRST LIVE ORB-via-http (#277)** — pre-open check + handoff watch + rollback per `docs/ops/monday_split_golive_playbook.md` (calendar event + session-only cron e2ec6871 set). Then → apply #278/#279 + W4 DR/ADR closure (DR runbook for 2 svcs, per-svc uptime checks, nightly-staging-restore twofer, #258 db.py split) | task #256/#277 · plan `…/execution-intelligence-split-256.md` · ADR 0012 |
...
- **Stocks in Play** (detection · setups · entries · 9M · cooldown · flags · entry-techniques): #289 (general-anticipation sister detector — Pradeep's anticipation shortlist (COO/HYLN/ALHC/APPS/NTAP) = general post-runup tight bases our flag detector misses because our gates are tighter than his discretionary selection (COO fails _RUNUP_MIN_RATIO=1.50 at +15%; ALHC fails _BASE_AGE_MIN_WATCH=3 at base_age 2). Fix is NOT a threshold drop (floods) but a SELECTION/RANKING layer: loose post-runup universe → rank by tight-base quality (RMV + base maturity + catalyst materiality) → surface top-N. Build as its OWN detector SHARING #270's lifecycle/RMV infra (per-setup judges, shared elements); shadow-first ~2wk vs Pradeep shortlists; includes the runup-floor + base_age 3→2 calibration eval (N≥30 + sign-off). Diagnosis durable: `docs/analysis/delayed_ep_composition_270.md` Anticipation Universe section; operator-decided this session) · #271 (Pradeep 20%-in-5d breadth extreme detector, two-direction timing: extreme-high -> shakeout caution, post-washout -> rally-watch; the CLASS B color rule exists but the metric feed was never computed. **ANALYSIS DONE 6/14 gate-free** (`docs/analysis/breadth_classb_271.md` + `scripts/_271_breadth_classb.sql` + `_271_analyze.py`): metric computed over 269 days, universe drifts 10% -> normalize to % of universe, p90 convention thresholds (AMBER up20_$5>=2.7%, GREEN washout down20_$5>=1.8%), validated against Pradeep's 5/24 anchor (lit AMBER 5/26-5/29, peak 5/27 = #3 day in 13mo). Two-directional BY CONSTRUCTION = clone of t2108_color, NO transition state (advisor: it's a DISPLAY ANNOTATION not a trade gate -> no forward-return backtest). Remaining = WIRING (feed in calculate_breadth_full + class_b_color + surface in BOTH briefs morning+evening + the UNIFIED /regime — operator 6/14: "this should be in the briefs" AND "merge /breadth into /regime, keep it simple" AND "/regime is not in the command list, needs adding" → fold the /breadth 10-day matrix into /regime + ADD BotCommand("regime") to the visible menu (currently a deliberately-hidden back-compat command, telegram.py:1660 — with /breadth dropped it MUST be discoverable) + DROP the /breadth command (3-place change, telegram.py orchestrator-owned); morning brief renders no breadth block today so it needs a new compact cell). **WIRING DEPLOYED 6/14** — operator relaxed the pre-Monday freeze for this market-independent change (orchestrator + intelligence-side only; no trade/boot-path impact) AND pinned the §C Monday rollback to a validated SHA (f116fae) so main can advance freely without endangering the fallback. Merge live (80156d3) + CLASS B feed/cells live (f116fae), both GREEN by the intelligence-role manual criteria (creds-less preflight false-fail is documented/expected; SERVICE_ROLE=intelligence, kept 42, 0 boot errors). VERIFY-LIVE PENDING: cells render once breadth_monitor carries up_20_5d (next calculate_breadth_full run) — confirm /regime shows the 5d±20% cell. turnkey spec in the doc) ? #270 (**RESET 6/16 → ADR 0013: Family A "consolidation plays post a runup"; the +40% phantom universe gate root-caused (reverse-engineered from MNTS alone), both shadow jobs PAUSED (commit 97f9486, pending operator-run `deploy.sh market-agent`); Phases 1-5 GATED on operator sign-off of ADR §2 criteria; #289 folds into Phase 2; EP family split to #297.** Prior scope: delayed-EP re-entry detector, MNTS template 6/11 ? composed gap?burst?low-vol-pullback?undercut?RECLAIM sequence, shadow-first; folds the U&R-invalidation fix: the flag detector disqualifies on the exact undercut that IS the setup; universe includes sub-$500M for observation. **STEP 1+2 DONE 6/14**, gate-free (`docs/analysis/delayed_ep_composition_270.md` + `scripts/_270_delayed_ep_replay.py` + `_270_cohort_run.py`). Step 1: composition state-machine validated vs MNTS (reproduces WATCHED 5/26 / ARMED 6/08-undercut / TRIGGERED 6/11-reclaim). Step 2: cohort of 134 price-action-seeded huge-gap names → funnel WATCHED 62 → ARMED 30 (48%) → TRIGGERED 16 (26% of watched = SELECTIVE); fwd MFE median +8%, ≥+20% on 29%, best +137% (the fat tail is present) BUT weak close-returns = empirically confirms the template's DERISK-FASTER nuance (edge in the excursion, not buy-hold). Calibration knobs surfaced for operator (EXPANSION unstable on near-zero baselines; thin-trigger-volume floor; GAP/VOLX/ARM defaults reasonable). **E2E REFRAME (operator 6/14, memory user-sip-setup-is-one-e2e-tactic)**: #270 is the READINESS layer of ONE end-to-end trade tactic, built as a single unit NOT fragments — readiness (done) + **INTRADAY ENTRY tactic** (step 2b, tunable, as important as the exit; first-min-hold/ORB-of-reclaim/U&R, needs intraday bars) + **EXIT/harvest** (W3, paired — the cohort's fat-MFE/weak-close result is the why). Ready transition is rare (~1/wk) + actionable → operator ALERT (not #168 per-tick noise). Step 2b = intraday entry tuning + Step 2c = ANTICIPATION entry (Pradeep EOD coiled-day) & MATURITY gate, both DONE gate-free 6/14 (`_270_entry_replay.py` + `_270_anticipation_replay.py`): anticipation +2.9R stop-and-reenter, does NOT beat confirmation on parity-clean R (edge = lower/earlier entry), and a >=3-day MATURITY base keeps all winners while collapsing outlier reliance 73%→40% — the WHEN answer; coil-maturity chart-read to integrate with the chart-vision renderer in the Judge bucket (build it as SHARED infra, 2nd consumer); **W3 EXIT (analysis done 6/15): scale-out +1R/+3R = robust PRIMARY shadow + two-phase (Pradeep day-0 trail→hold survivors, 73% win) SECONDARY shadow; CATALYST-CONDITIONAL leash REOPENED (operator 6/15 — the "refuted" read was confounded: cohort catalysts are historical-scan PLACEHOLDERS not real grades; 57.7%-gap MXL had an UNKNOWN unsourced catalyst) → re-test once the catalyst-sourcing backbone (210/211) sources the cohort properly; do NOT ship catalyst-into-exits until then.** Step 3 = deployable SHADOW tactic (readiness table + job + entry-watch + /-board + alerts) GATED until after the Monday live-ORB gate closes — a new scheduler job + CREATE TABLE run in combined = the §C rollback target. **Shadow→actionable GRADUATION TRIGGER pre-registered (operator 6/14)** in `data_gated_reviews.yaml::delayed_ep_270_shadow_graduation`: predicate counts settled shadow triggers, **N≥5 = early directional peek (operator 6/14; hold-for-more-data via deferred-status is the designed default)**, auto-surfaces in the Sunday digest — verify the predicate vs the shipped `mi_delayed_ep_lifecycle` columns at step-3 ship (build spec carries the checkpoint). **CALIBRATION knobs #1/#2 PROBED 6/14** (`_270_calibration_probe.py`, N=17 illustrative, pending sign-off: #1 floor EXPANSION base 0.5×ADV20 cosmetic + keep 1.5× gate; #2 cohort REFUTES a trigger-vol floor — SILO +37% thin winner) + **re-validation registered** (`data_gated_reviews::delayed_ep_270_calibration_revalidation` N≥30 + N≥5-peek cross-ref + Monthly-sweep wiring at table-ship, operator "include in regular reviews")) ? #55 #56 #60 #65 #81 #97 #113 #115 #116 #134 #146 #166 #167 #168 #170 #178 #209 #218 #220 #226 #227 #229 #283 #284 #285 #286 (#286 = RS leaderboard saturated by illiquid micro-cap pumps — operator caught 6/15 ASTC #3 RS = a $2.47→$49.80→$16.91 micro-cap pump-fade, rs_1m=100; whole top-8 is illiquid pumps incl. ELOX #1 at ~$0/day ADV$. Root cause = RS ranks ~9700 on pure return percentiles, NO liquidity/cap floor + market_cap=None unpopulated. Gated fix = $-vol floor + populate market_cap + optional fade guard; confirm rs_engine.py gap then backtest N>=10-30 + sign-off, no self-certify) (#285 = graduate the recurring M&A filter accuracy review [scripts/mna_filter_accuracy_review.py + data_gated_reviews mna_filter_accuracy_review, monthly] into the monthly backward-check sweep auto-run+Telegram; GATED post-Monday, scheduler/combined) (#284 = M&A filter acquirer-direction TITLE leak fix — ONDS/MYRG-class acquirer-side title-match suppression; built+backtested 14/14+16 unit tests 6/14, AWAITING operator sign-off [HARD-gate filter list] + shadow-validate, deploy after Monday go-live; SUNE Path B roundup-bleed deferred under mna_filter_direction_blindness_path_a) (#283 = wick_fill promotion eval shadow->live, POSTPONED behind #270 per operator 6/14 — wick/U&R flagged most-important entry technique but finish SiP first; promotion = CHANGE_PROCESS + R/win-rate evidence + sign-off, NOT the raw candidate-count "ready" flag the weekly digest auto-shows) (#229 = EP gap_pct accuracy: ABVX 06-03 recorded +15.1% but reality was ~-44% gap-down — detection-recording bug; #227 = fix scripts/probes/test_9m_ep_e2e.py UnicodeDecode+DB errors, low-urgency test-harness bug filed 6/6; #226 = R3 persistent narrative→mi_themes canonization (Lane 2 MERGE) — DEFERRED per 6/6 advisor review: blocked_by task-214 naming stability + post-Monday 200-verify, ETA ~6/16, advisory-only NOT load-bearing (daily re-assert + theme_pass1_5_absorption dedup + fade lifecycle); #209 = leveraged/inverse-ETF universe hygiene; #218 = command-surface consolidation — /detectors roll-up for the 5 shadow detectors + drop redundant aliases + fix /ep|/eps, operator review 6/6; #220 = merge /watch + /inplay into one /watch (keep /watchlist) — the #168 actionability filter defines its default; **#168 NOISE FIX shipped 6/7**: 5 intraday entry-technique shadow detectors were per-tick-pinging ~23/day ("telemetry only") — flag-break unguarded + 3 env-gated (prod `SHADOW_DETECTOR_TELEGRAM_ENABLED=true` overrode the "silenced 5/26" intent) + U&R already off. Now per-tick DEFAULT FALSE in code (SoT, survives DR/env-reset) + ONE 16:00 ET consolidated digest `run_intraday_signals_eod_digest` (suppress on zero, cites /detectors). DB+audit untouched (N≥10 eval intact). REMAINING #168 = the quality/actionability filter for which signals earn a LIVE ping at graduation (evidence-gated, needs the outcome data shadow is collecting)) · #297 (FAMILY B — EP family rework: delayed-EP / MAGNA53 / 9M, EP-REQUIRED (fishhook retired 2026-07-21 — was one implementation of the delayed-EP concept, which survives); the NEXT & SEPARATE rework after Family A, its own scoping session; reuses the U&R generic mechanic from Family A but owns its universe/gating/stops; ADR 0013 §5 out-of-scope; filed 6/16)
- **Live-money cutover** (MAGNA53): #150 (↓DOWNGRADED 6/3 — entry-tuning, not safety) #151 #183 #184 #224 #225 #275 (**#275** = kill/scale band evaluation in Sunday digest + kill_scale_override mechanism — the two operator sign-off conditions made mechanical; runway Fri 6/19, must exist before live trades accrue; quarterly band review separately registered `data_gated_reviews::kill_scale_bands_quarterly_review` first 8/1) (**#268 ✓ DONE 6/12 — v1.1 W1: Phase B replay 1307/1307 judged (floor +0.90R vs judge +0.95R; promote cohort +1.26R/33% = the consistent edge; demote side = standing watch-metric) + kill/scale bands SIGNED into safeguards.md with operator conditions (quarterly standing review; explicit logged override authority);** #183 = ORB classifier IEX/window mislabel; #184 = broker-authoritative trade-state mirror (ADR 0008) — **increment 1 demotion FENCE shipped 6/6** (`audit_trade_state_demotions.py`), increments 2/3 pending; #225 = resolve the 3 demotion residuals (L584/905/1575 order_manager) the fence surfaced, paper-gated, then wire blocking deploy gate; #224 = SIP-replay robustness checks. **#223✓ DONE 6/6 — Gate-3 SIP-selection verdict: paper −$9,475 is an IEX feed artifact; +2.27R same-basis selection delta; GO-supportive for reduced-size 6/22. Doc `docs/analysis/sip_replay_gate3_2026-06-06.md`.** Done & dropped: #174 #180 #181 #182 #223.)
- **Judge & catalyst (ex-North Star → v1.1 program)**: #276 (**W2 entry-mechanics program** — harness + full-year ATR coverage ready; study #1 done 6/12: 5-min-OR dead, skip-wide-open NOT-shippable (outlier-concentrated); next = Mon stop-geometry study; carries the wide-open-as-context-input + first-pullback parked ideas) #274 (2-member theme immortality: below-2 guard skips removals so misassembled pairs are unfixable - ECOR/FEIM 2mo case; fix = dissolve-on-flagged-pair, CHANGE_PROCESS + would-dissolve list to operator; couples w/ #266) #272 (LAUNCH 6/22 umbrella, SSoT docs/roadmap/launch-2026-06-22.md) #149 #165 #191 #192 #197 #219 #236 #266 #269 (**🏁 #240 CLOSED 6/11** — verified-live a day early: 3 Opus-judged alerts, exact alert↔decision↔authority reconcile, 0 fail-opens; #242–#245/#247/#249/#252/#253 all closed-verified; ADR 0011 IMPLEMENTED-LIVE; CHANGELOG 2026-06-11 has the full record; remaining tasks here continue under the v1.1 program, NOT a reopened North Star) (**JUDGE WENT LOAD-BEARING Wed 6/10** — operator-signed flip after the CBRL first-fire demote (one-time litigation settlement, reviewed clean); **#189 #200 #201 #202 #203 CLOSED-AS-SUBSUMED 6/10 via #249** — the advisory stack (materiality_shadow, theme_gated_*, _compute_fire_status fire panel) retired into the one judge; judge verdict fire_axes is the fire signal, ADR 0010 superseded, weekly review carries one judge roll-up section; #202's discovery-latency goal carried by #211/#235; #190 was already done) (**#240 = EP Holistic Grade Judge umbrella, ADR 0011; #241 W0✓ DONE; #242 W1 judge shadow-emit shipped; #243 W2 judge-supersedes-floor SHIPPED DORMANT (toggle OFF, flips 6/9); #244 W3 review surfaces (sweep + delta-review) built; #245 W4 consolidate advisory stack — flip-gated. Committed acceleration: North Star DONE 6/13; SiP→judge expansion DEFERRED per operator, per-setup judges not one universal**) (#236 = make corpus_provenance a BYPRODUCT of build_grounded_text — the grade-input/source-class decision is split across 3 lockstep sites (grader fallback + build_grounded_text membership + corpus_provenance re-derivation); refactor to {text,sources} so #211 KPI can't silently miscount; /simplify 6/7 altitude finding, do AFTER Mon 6/8 verify + as Wave-D #234 groundwork; **#219** = weekly verified-miss review — systematic missed-winner analysis from /missed, with #197/#199; **#197 = cap+1 game_changer slot SHADOW shipped 6/6 (`shadow_cap_plus_one_197.py`), promotion-gated N≥30+sign-off; #198 CLOSED obsolete 6/6 — deprecated circuit_breaker, tiered drawdown breaker already covers it**) · **theme-validation correctness cluster**: **#213✓ DONE 6/9 VERIFIED-LIVE** (false-removal — shield+Haiku→Sonnet shipped 6/6; Mon 6/8 17:00 ET run clean on claude-sonnet-4-6, zero errors/shielded events, SNDK+SIMO retained in 'AI Memory & Storage'; 16 removals incl. 13 majors out of 'Pure-Play Hydraulic Fracturing' = the eval-predicted XOM/CVX class, list surfaced to operator not self-certified) → #214 (theme auto-NAMING defect — names narrower than the RS cluster, the oil&gas removal root cause; **SHIPPED 6/9 pending verify**: name-breadth-must-match-members rule in all 4 naming surfaces (discovery prompt/schema, split prompt/schema, Lane-2) + audit-only `validation_mass_removal_name_suspect` tripwire; misnamed frac theme already Retired/empty so no data fix; verify = next discovery runs' names + cooldowns_per_day back to ~3/day baseline) → #215 (surgical prompt de-bias / OPTX-residual — BLOCKED_BY #214 + needs a clean two-sided cohort + CHANGE_PROCESS; earliest after #214 lands) → **#266** (event-driven membership validation at theme BIRTH + identity-change, M/W/F demoted to drift backstop — operator architecture question 6/10 off the DDOG removal: entered at birth 3/20, sat mismatched 12wk, caught only after #213 model upgrade + #214 naming sharpened; CHANGE_PROCESS + replay-evidence gated) · **#267** (judge CHART-VISION input — point-in-time matplotlib renderer from mi_daily_closes + image block in grade_holistic + chart-structure rubric axis; with-vs-without replay + operator labels per the model-flip playbook; post-North-Star feature bucket, operator-directed 6/10. **2nd CONSUMER = the delayed-EP anticipation coil-maturity chart-read (Stocks-in-Play bucket, operator 6/14): build the point-in-time renderer as SHARED infra, not judge-only — the anticipation `base_run` gate is the cheap pre-filter, chart-vision is the richer maturity read. Today's charts.py = Finviz live mosaic = lookahead-unsafe, not reusable.**) → #217 (query hygiene — **SHIPPED 6/9 pending deploy**: protected set fetched once/run + threaded through rescore/assign into validation (was 1 query/theme); run-level cooldown fetch reused for assignment (2→1). Deliberately NOT fully collapsed: the cooldown fetch must stay post-rescore — Mon/Wed/Fri validation ADDS cooldowns the same-run re-assignment guard must see) · **catalyst-sourcing backbone #210** (operator direction 6/5: DIRECT primary sources > LLM-discovery — umbrella over #187✓/#208✓/#191; PARKS #186) — **SCOPED 6/7 into Waves A-E** (`~/.claude/plans/catalyst-sourcing-backbone-210-scope.md`): **#231 Wave A** (wire Benzinga/Alpaca news into corpus + primary-subject relevance filter #88/#90 class — SHIPPED+DEPLOYED 6/7 `cbb7619`, grounded-only/activates Mon 6/8 scan — **NOT telemetry-safe**: Benzinga text feeds grade→ep_score→score_tier→HIGH→phase=paper ORB entries, so risk = systematic grade inflation on PR-having tickers; GRRR 6/2 validation `docs/analysis/wave_a_benzinga_2026-06-07.md`: GOAL = attribution CORRECTNESS (not winner-rescue/P&L — separate axes, memory `feedback-catalyst-correctness-is-the-goal`). Correctness DEMONSTRATED (isolated +1 tier strong→game_changer w/ correct sizing, lower-bound) + 6-K-lags-3d proves press-wire timeliness. Winner-rescue = supplementary evidence only, NOT a gate. Remaining: systematic correctness at scale via #211/#202 + Mon 6/8 first-fire verify) → **#232 Wave B** (source provenance — measurement enabler for #211; `corpus_provenance()` emits `ep_catalyst_provenance` audit per graded ticker: {sources:{class:count}, has_direct_source} mirroring the grade's INPUT BRANCH (fmp-fallback→`fmp_aggregator`, not `{}`); has_direct_source structural `sec_*|benzinga_pr` (Wave-D 425 safe). BUILT+18 tests 6/7, DEPLOYING 6/7 with a self-verifier: the 10:05 ET `_ep_scan_watchdog` now reads today's `ep_catalyst_provenance`, writes an `ep_provenance_daily` KPI audit row (graded/direct/unknown + by-class), and Telegrams 🔴 ONLY on the broken case (alerts but 0 provenance = silent path death). Monday verify = retrospective read of that row — provenance is also the observability that proves Wave A's Benzinga wiring fired) → **#233 Wave C** (demote Perplexity/Tavily to labeled candidate, blocked_by 231) · **#234 Wave D** ✓ DONE 6/7 (EDGAR breadth — shipped the tradeable core: 8-K EX-99.1 CONCATENATION for item 2.02/7.01/8.01 or thin primary, validated CRM/AVGO/COST/HPE/CRWD; 425-M&A DEFERRED = skip-only-upside + direction trap; foreign-issuer SKIPPED = 6-K done; NOT telemetry-safe → Monday inflation check covers) → **#238** (the REAL higher-value EDGAR gap: NEGATIVE-catalyst dilution filings S-3/424B5/8-K-3.02 for two-directional DOWNGRADE — the GRRR debt-drop class; feeds the #189 holistic judge) · **#235 Wave E** (gap-discovery loop #211 integration, blocked_by 232) · **#230** (#212 as sourcing-QA detector → feed #211: "hard gap + grounded no-catalyst" = sourcing-gap flag; LOW-precision triage) · **#211 news-gap discovery loop** (unknown-rate = source-coverage KPI; DEFERRED post-Monday — needs a new scheduler job + fire_status data that lands 6/8) · #208✓ (EDGAR 6-K foreign-filer gap — DONE 6/6) · **#212 questioner/investigator dialogic loop** (PROTOTYPE DONE 6/6 — mechanical grounding > LLM skeptic-PM; productionize post-Monday at larger cohort) · **model-eval governance #207** (quarterly review, auto-surfaces via `data_gated_reviews::model_selection_quarterly_review` 2026-08-01; SSoT `docs/model_selection_baseline.md`) · #186✓ (Gemini LLM-discovery — EVALUATED→PARKED under #210, DONE 6/6). (#197/#198 = quality-aware admission / conviction-override; re-bucket to Stocks-in-Play if preferred.) · **Judge follow-up cluster #247 #249 #251 #252 #253 #255** (filed in-tracker 6/8–6/9, backfilled to this view 6/9 — the session that created them was cut by a Claude update before the BACKLOG mirror): #247 (consolidate judge DB writes — atomic merge + one-time column-ensure; W2/W4 /simplify deferral) · #249 (post-flip: retire the advisory stack — materiality_shadow / fire-panel compute / theme_gated_* — into the one judge) · **#251✓ DONE 6/9 VERIFIED-LIVE (rule_materiality false-transformative noise — extract_deal_value now DEAL-CONTEXT-AWARE: metric veto + deal-keyword require, abstains on earnings revenue/guidance figures → judge owns materiality. Commit `269f33c`, deployed; prod probe over 165 alerts/45d: deal-value fired 99→28, KSS $3.17B-revenue→abstain. PGY kept $2.1B = correct residual: genuine "secured $2.1B ABS funding" transaction — routine-for-Pagaya is judge-context, and the judge already overrode it)** · #252 (judge model selection: Sonnet-vs-Opus diverge on THIN input, artifact-suspect — grounded re-run is the test) · #253 (judge --grounded over-demotes web/theme-driven catalysts, RCAT class — replay lower-bound caveat + theme-axis blank root-caused to Lane-1 missing the real drone cohort; motivates Lane-2 on the judge's theme axis) · #255 (ground-truth Phase 2 — judge-precedent retrieval kind='review' ONLY + periodic-review nudge + FSLY pre-coverage vs in-coverage-miss refinement; advisor-gated)
- **Operational safety / hardening**: #288 (RMV-miss mitigation Prong B — deterministic idle-review escalation: any data-gated review past-floor+over-threshold (READY) OR with an ERRORING predicate (the #54 silently-broken-locked-query class) beyond a 7d grace emits its OWN deduped Telegram, folded into `_post_nightly_audit_job` + state table `mi_review_escalation_state` + on-demand `/datareviews` command. SHIPPED+DEPLOYED this session (26f04a2+bce181c+89e377e); 6/15 follow-up reused the canonical `_md_escape` (#148) instead of a 4th escaper [/simplify catch] — that escaper-consolidation fix is UNCOMMITTED-then-committed this close, redeploy `market-agent` before the ~6/23 grace-crossing fires the first real escalation. Prong A [primitives catalog + search-before-build] = DONE this session. Root of the RMV-miss class) · #290 (fix `dead_zone_reevaluation` broken predicate — `column dc_fwd.high does not exist`, #54-class silently-not-ready; surfaced by Prong B's first live run; correct the column ref + re-eval) · #291 (triage the 26 idle data-gated reviews Prong B surfaced as READY-but-rotting [incl. #54, paper_r_expectancy_validation, gap_atr_3_5x_band_negative_ev, fishhook_v3, flag_proximity_*, drawdown_breaker_promotion, several live-cutover gates] — run / defer / ratify-close each so the escalation list converges; #54 = operator ratify the close, verdict RMV redundant +0.17pp) · #273 (LLM credit-exhaustion alerting - core sites shipped 6/11 night after Phase B exposed the silent-fail-open billing gap; remaining = decorator-hook sweep + spend telemetry + low-balance warning) #152 #172 #287 (partial-exit trade-state cleanup — FPS #183 stuck partial_taken=TRUE/empty-exits locks it out of partial [pre-fix optimistic-write artifact; current finalize_partial_exit correct]; + exits double-encoded as jsonb-string in 8 May trades [app-resilient, breaks SQL]; surfaced 6/15 #151 calendar check; trade-state mutations operator-gated) #176 #179 #216 #237 #239 #256 #277 (**⭐ Mon 6/15 split go-live verify** — #256 CUT OVER 6/13, live on paper as 2 services; FIRST live ORB-via-http: pre-open check ~5:57 PDT + 9:31 ET handoff watch + rollback per `docs/ops/monday_split_golive_playbook.md`; durable backstop = this task + Google Calendar event + session-only cron e2ec6871) #278 (**deploy.sh preflight role-aware** — post-cutover `deploy.sh market-agent` runs the entry-pipeline safeguard preflight against the now-creds-less INTELLIGENCE container → spurious "PREFLIGHT FAILED" even on a perfect rebuild (verified false-fail on the 6/13 timeout-split deploy 3cb4b9c; manual green-criteria in `execution_split_cutover.md`). Fix = make the preflight role-aware: target `apollo-execution`, or skip with a clear "intelligence role — safeguards run on apollo-execution" message when the scoped container resolves to intelligence/no-creds. Monday rollback UNAFFECTED (combined has creds → passes). Post-Monday, low-urgency) #280 (**staging shared paper-account isolation** — W4; `.env.staging` copies prod's PAPER Alpaca creds so staging-execution acts on the SAME paper account. Scheduled mutations gated (LIVE_TRADING_ENABLED=false + streams off), but operator MUTATING commands to the staging bot (/syncnow·/partialnow·/timestop) bypass that gate → would hit prod's paper account. Interim: banner warns read-only/off-hours. Fix = staging its own Alpaca paper account (or creds-free) + verify the gate covers cancels/replaces not just submits. Gate: before any market-hours staging use) #281 (**staging copy-not-derive hardening** — W4, from the W3 /simplify: staging COPIES 3 prod artifacts that can silently drift — (1) restore fork: staging_seed.sh clones infra/restore.sh's recipe (EXPECTED_ROLES drift; "rehearses DR" false while forked) → extract shared restore_db(), DR-aware sourcing; interim cross-ref comments added; (2) compose-by-copy: a new prod env/service/healthcheck never reaches staging (limits divergence is deliberate cap-low, commented) → override layer or drift-check; (3) .env-by-copy: new required prod var silently missing → env_file inherit + overrides. Build-once fix already applied) #279 (**W2 split /simplify deferrals — apply post-Monday** — 4-agent review of the 62aeba6..3cb4b9c split diff: diff clean on efficiency + largely right-altitude, but APPLY NOTHING until after the Mon 6/15 live-ORB verify (all findings touch now-live code → forced redeploy). Findings: (1) derive `_EXEC_HANDLERS` from `_CROSS_FNS` via getattr (deletes the hand-maintained 2nd manifest + parity guard, fails loud at boot); (2) make the scheduler partition omission guard BIDIRECTIONAL (`registered == EXEC ∪ INTEL`) so a stale intelligence-job id can't sit silently dead — the static verify_job_partition.py is wired into nothing; (3) `get_data_feed_name()` duplicates `alpaca_client.get_data_feed()` env resolution (benign drift) → extract a broker-free shared resolver; (4) minor: pooled httpx client, `auth_headers()` reuse, `_SLOW_COMMAND_FNS` maintainer comment) #257 #258 #259 #260 #265 (**#265** = 6/10 advisor+simplify review residuals over the judge-flip-day diff: preflight_db_updates SQL-text SSoT hoist · shared ingested-feeds list (news_source_quality ↔ source_gap_finder) · merge the two mi_theme_candidates_shadow writers · WATCH judge-write cancellation window (advisor A3, ms-wide, >12-alert mornings); the load-bearing review fixes were applied same-session · **2026-06-09 Fable 5 architecture-review cluster + operator staging directive**: **#256 BIG ROCK** = execution/intelligence service split + STAGING pipeline before live money — operator-requested; needs its own scoping session, split targeted before 6/22; ⚠ propose adding to the Active Major Projects roster · **#257** = central LLM model registry `shared/llm_models.py` + literal-ban deploy gate [5i/7] + FIRST CI workflow — SHIPPED 6/9 `c6ed3f5`+`39ff49a` (theme advisor opus-4-6→4-8 flip; Opus pricing rows were 3x stale at $15/$75, actual $5/$25), verify = next deploy [5i/7] + first Actions run + advisor_call logs 4-8 · **#258** = boot-time ensure_schema() consolidating the scattered ADD-COLUMN sites + db.py domain-package split — generalizes the judge column-ensure cleanup (task 247 part 2, filed under North Star). **AUDIT DONE 6/13** (`docs/analysis/schema_consolidation_258.md` + `scripts/_schema_consolidation_audit_258.py`): of 43 live `ALTER ADD COLUMN` sites — 10 REDUNDANT (drop-safe), 33 FOLD-into-CREATE, 0 KEEP/0 ANOMALY (prod fully consistent → consolidation is mechanical: fold 33 + drop 43). ⚠ **The EDIT is GATED post-#277**: `initialize_schema()` runs in combined, and §C Monday rollback = `git pull main` → rebuild combined, so the edit must NOT enter main's rollback path until the gate closes (advisor 6/13). Step 2 = consolidation edit (branch + staging-validated + preflight_db_updates); Step 3 = the db.py domain split (own session) · **#259** = failure-policy decorators (@advisory_fail_open / @trade_state_fail_loud) — make the 492-site except-Exception policy declared not re-decided · **#260** = freeze the execute_task keyword cascade: routing regression test now, orchestrator-side intents later, merge the 3 ticker skip-sets · **#263** = nightly_data_pull 'empty_result' two consecutive nights (~3.9k rows vs 5k floor, nightly false/real-alarm Telegram) — diagnose stale floor vs missing writer, filed 6/9 · **#264** = news-source-quality drift check needs min-N floor + composition context — 6/9 'Benzinga 68%→20%' alert diagnosed FALSE ALARM (n=10, earnings-season-tail cohort cites Perplexity; alpaca content present 7/10; will re-fire daily on thin weeks until fixed)) (**#205✓ DONE 6/7** — closed the 3 redesign-needed stale tests: test_returns_themes_list skip was stale + a _get_anthropic_client global-cache leak (patch the getter, not AsyncAnthropic); 2 theme-notification tests reworked with real-ticker fixtures + mocked RS getters + 'THEME ENGINE — N active'. Full suite 553 pass / 1 env-skip; #239 = 6/7-late /simplify deferrals, both post-Monday: (a) Wave-A trailing-baseline dup across verify_monday_firstfire ↔ _wave_a_grade_inflation_check — extract to a DURABLE home (not import-from-disposable) when _wave_a graduates/3rd consumer; (b) `_emit_shadow_row` envelope helper for run_ep_scan's per-ticker shadow emits (tape/perplexity_boost/provenance) — defer, hot-path first-fires Mon; **#221✓ #222✓ DONE 6/7** — deploy.sh governance/docs/tests deploy-irrelevant + missed_outcomes duplicate_scan SQL DRY'd to one constant; #237 = extract a shared scheduled_eod_digest helper for the ~6-job 16:xx digest family (window/dedup/rank/cap-overflow/suppress-empty/trading-day-guard) so they can't re-diverge — /simplify 6/7 altitude finding; the two concrete divergences it surfaced (both digests at 16:00 → staggered to 16:20; 9M pace missing trading-day guard → added) were FIXED inline 6/7, this is the abstraction follow-up, low-urgency, gated post-Monday-first-fire; #172 = partial-exit /simplify refactor, gated behind the partial-exit hardening architectural split; #176 = automated SoT/roster drift-check — reconcile CORE + pre-commit Gate 2 SHIPPED 6/6, remaining = create-time hook + scheduled ping; #205 = stale-test fix; #216 = jsonb codec consolidation — root-fix of #179's per-site _jsonb_param, high blast radius, gated past 6/22)
...
- **Miscellaneous** (no home project): #121 (briefing → shared Telegram formatting layer. **Concrete scope surfaced 6/15 /simplify:** ≥3 Telegram-Markdown escapers coexist — `_md_escape` (#148, in `scheduler.py`, escapes `_`/`*`, deliberately KEEPS backticks for code spans; parabolic_detector reaches into scheduler to import it) · `shared/telegram_format.py` (HTML-mode, different parse_mode) · and a 4th briefing-homed one I started before finding `_md_escape` then reverted to reuse — the search-before-build miss this layer prevents. Consolidate to ONE escaper in a neutral home, RECONCILE the backtick policy [the contracts genuinely differ — can't blind-merge], and re-home off scheduler.py so detectors don't import a string util from the scheduler) · #195 (SECURITY — rotate portfolio-app2 app_password; operator-action) · #228 (weekly-review narrator: stop mis-flagging reentry-block-after-fill as "traded through" — 3wk recurring false alarm, verified 6/7; low-pri review-quality) · #246 (theme_engine.py:3011 isinstance(anthropic.APIError) TypeError in local test env — not prod, not in judge diff) · #248 (regime engine: post-pullback caution/hysteresis term + spy_vs_200ma NULL fix + stale Bull=70 docstring; filed 6/8, backfilled to this view 6/9) · #261 (scripts/ namespace reorg — 172 files: split ops/evals/probes, one mechanical PR w/ path-reference sweep; 6/9 review hygiene tier) · #282 (README.md:791 slash-command list stale — lists /positions·/eps·/9m·/themes·/clusters·/regime, none current BotCommands; operator caught /positions in the W3 staging smoke 6/13; low-pri doc reconcile, not load-bearing — parity gate keeps the real commands in sync) · #262 (CLAUDE.md over its 40k hard ceiling since ≥5/29 — graduate Recent entries to CHANGELOG + cut mechanically-enforced prose + consider pre-commit char-count warning; promoted from 5/29 closer-prose on 6/9 operator audit. **Ceiling-gated edits owed (apply during the next compression pass, file is currently +296 over so they can't land yet — filed here 6/15 instead of left as commit-prose):** (1) the "Adding a Telegram slash command" section says **THREE** places — it's **FOUR**: the `channels/telegram.py` CommandHandler *registration loop* (~:1636) is the ACTUAL route; `BotCommand` only populates the `/` autocomplete menu. The 6/15 /datareviews bug (command "did nothing") was exactly this — registered the BotCommand, missed the handler-loop. (2) a "Changes Made — Recent" bullet for the RS liquidity/cap floor change (the RS-universe exclude-illiquid-micro-cap-pumps work, ASTC/ELOX class — filed as a task under Stocks in Play). (3) a "search-before-build a methodology primitive" rule + pointer to the new `docs/methodology/primitives.md` catalog (Prong A) — grep + consult the catalog before inventing a new metric, so the RMV-miss class can't recur)
...
### ⭐ Mon 2026-06-15 (~5:57 PDT / 8:57 ET) — #256 split GO-LIVE verify (#277)
The execution/intelligence split was CUT OVER 6/13 (operator-authorized, markets
closed) and is live on paper as two services. **9:31 ET (6:31 PDT) is the first
live ORB routed through HTTP** (ep_scan → trigger_orb_entry → /exec →
execution._orb_monitor_job) — the one path the markets-closed soak can't test.
**Playbook: `docs/ops/monday_split_golive_playbook.md`** — §A pre-open check
(~30 min buffer), §B handoff watch, §C rollback (instant, byte-identical to
combined). Durable backstop (cron e2ec6871 is session-only): **this entry + the
Google Calendar event (Mon 5:55 PDT)** + the committed playbook + #277.
...
- [x] `fishhook_v3_first_telemetry_review` → YAML, retired 2026-07-21 (fishhook removed)
...
- [x] `fishhook_v3_promotion_check` → YAML, retired 2026-07-21 (fishhook removed)
- [ ] `rs_theme_dash_forward_returns` (10/1) → YAML
- [x] `fishhook_TI3_revisit` → YAML, retired 2026-07-21 (fishhook removed; TI3 #314 needs a fresh non-fishhook approach if pursued)
...
- [~] **P21** Cross-asset thematic RS — V1 script shipped tonight (`scripts/probes/cross_asset_rs.py`); V2 conviction boost into theme_engine deferred (needs theme-to-commodity mapping) → memory
...
- [x] **TI3** Fishhook V3 — RETIRED 2026-07-21 (operator: discretionary delayed-EP re-entry doesn't belong in the automated core); a fresh non-fishhook approach would need its own scoping → memory
...
- [~] **TI5** Post-EP Pattern Shape Classifier — explorer script ready (`scripts/probes/ep_shape_explorer.py`, 376 lines), v0 cohort review pending → memory
