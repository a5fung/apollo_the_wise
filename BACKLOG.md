...
- [x] **MRAM methodology-attribution fix** ✅ APPLIED 2026-05-18 — tagged MRAM #120 with `pnl_attribution='incident_2026_05_11_mram_phantom_double_exit'`. Phantom double-exit (-$2,199.71 vs methodology-correct -$1,101.93) excluded from Gate 3 methodology metrics. Actual P&L still hit account; only methodology-evaluation queries filter on this column. Note: MRAM was already pre-cutoff (alert_date 5/11 < Gate 3 cutoff 5/12), so today's Gate 3 cohort unchanged — tag documents the incident for future analyses.
