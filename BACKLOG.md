...
| **North Star** — theme/narrative + CATALYST load-bearing in EP | **P1** | C1 catalyst_type LIVE; #200 theme-gated + #201 fire-panel ADVISORY shipped (verify-live 6/8); catalyst-axis baking (#187 EDGAR✓ · #189 materiality · #190 grounded-Sonnet grade); model-selection BASELINED (#188✓); **#210 catalyst-sourcing BACKBONE stood up in SHADOW 6/7** (Wave A✓ Benzinga corpus+relevance-filter · B✓ provenance KPI+self-verifier · C-Part1✓ Perplexity-boost shadow · D✓ 8-K EX-99 concat; E + #238 dilution-sourcing + demotion/flip gated post-Monday); **OPERATOR DIRECTIVE 6/7: gap-alone-≠-HIGH, real catalyst LOAD-BEARING** (evidence engine `fire_status_r_cohort.py` shipped) | #200/#201 + Wave-A/B/C/D first-fire verify 6/8 (`verify_monday_firstfire.py`) · theme-as-EP-signal n≥40 ~8/1 · phase5 advisory composite · quarterly model-eval 8/1 (#207) · ⭐ phase6 GATING ~Q4 | memory `north-star…` (Track B) · `phase5/phase6` reviews · `ep-conviction-floor…` · `docs/model_selection_baseline.md` |
| **Operational safety / hardening** | **P0** | partial-exit hardening trilogy SHIPPED + paper-validated 6/1; 16:45 cron re-enabled & verified; timezone PERMANENT FIX shipped 6/5 (#206); **6/7: #205 stale-tests✓ (suite green) · #221 deploy ownership-map✓ · #222 missed_outcomes DRY✓ · CLAUDE.md graduated 58.9k→41.9k** | residuals: #152 news-drift · #172 partial-exit refactor (post-#151) · #216 jsonb-codec root (gated post-6/22) · #237/#239 /simplify refactors (post-Monday) · #225 demotion residuals · ZS silent-trigger obs | BACKLOG "🚧 Live-cutover blockers" + tasks |
