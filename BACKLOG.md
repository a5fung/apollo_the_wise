...
- [ ] **Track 1 T1.5a — `set_stop_order_id` helper consolidation** (2026-05-17, deferred per advisor) — 12 solo `UPDATE mi_live_trades SET stop_order_id = ... WHERE id = ...` write sites collapse to a single `set_stop_order_id(trade_id, new_id, *, reason, account_mode)` helper. Multi-column atomic closes (`status='closed', stop_order_id=NULL, closed_at=NOW()`) stay inline — splitting them would lose atomicity. Gate 5 G's allow-list tightens 25 → 1 entries for stop_order_id after consolidation. Per advisor: cosmetic-not-safety; Gate 5 G enforcement value identical with or without. ~40 min once T1.5 ship is stable.
...
- [ ] **MRAM methodology-attribution fix** (weekly review 2026-05-17) — logged -$2,199 vs actual -$1,100. Phantom double-exit from WS misfire after stop_order_id was null. Post-stop-ACK-timeout-gate ship (commit `8e8f6f3`), the gap class no longer recurs; the historical MRAM #120 row still shows the inflated loss. File one-shot reconcile script if accurate methodology cohort R numbers matter for Gate 3 paper R-expectancy validation.
