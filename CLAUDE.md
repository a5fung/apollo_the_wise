...
**🔒 A LIVE CHANGE DECLARES ITS EXPECTATIONS BEFORE IT SHIPS — GATED (operator 2026-09-13: *"we should make this standard practice, write down expectations to be verified live so we know how to treat what we see in real live data, what to do, and when we're done"*).** A `deployed` task states **`EXPECT:`** (what live data should show, against today's BASELINE, positives **and** unintended consequences together) and **`DONE-WHEN:`** (the stopping rule). Escape: `EXPECT-NA: <reason>`. **Written BEFORE the change — an expectation formed after seeing the data is a rationalisation**, which is exactly how five conclusions were reversed in one evening on 2026-09-13. Gated new/edited-only (`check_plan._expect_gate`); the pre-existing `deployed` lines surface as a counted backlog. Model: `docs/analysis/cross_industry_themes_2026-09-13.md` §PRE-REGISTRATION.
